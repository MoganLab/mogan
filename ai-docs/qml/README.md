...
正解是**快照写回**：对话框打开瞬间（register-specs，live 改动前）快照；Cancel/重置共用
一个 revert，再 `selector-get-changes` + **一次** setter 写回。一次 setter 避免多次
...
- **ParagraphFormat**（`generic/paragraph-format-widgets.scm`）：entry-key = `(int-句柄 var)`，
