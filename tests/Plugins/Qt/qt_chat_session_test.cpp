...
TestChatSession::test_setType () {
  ...
  // 不存在的会话静默忽略
  mgr.setType ("nonexistent", "explain");
}
...
TestChatSession::test_findSessionBySourceDoc_matches () {
  ...
  QVERIFY (is_empty (mgr.findSessionBySourceDoc ("doc-1", "explain")));
}
...
TestChatSession::test_findSessionBySourceDoc_distinguishes_type () {
  ...
  string sid_explain= mgr.createSession ();
  mgr.setSourceDocId (sid_explain, "doc-1");
  mgr.setType (sid_explain, "explain");
  QVERIFY (mgr.findSessionBySourceDoc ("doc-1", "translate") == sid_translate);
  QVERIFY (mgr.findSessionBySourceDoc ("doc-1", "explain") == sid_explain);
}
