...
struct virtual_font_rep : font_rep {
  ...
  void  draw_clipped (renderer ren, scheme_tree t, SI x, SI y, SI x1, SI y1,
                      SI x2, SI y2);
...
virtual_font_rep::virtual_font_rep (string name, font base, string vname,
    ...
    : font_rep (name, base), base_fn (base), fn_name (vname),
      ...
      extend (extend2), last (N (virt->virt_def)),
...
virtual_font_rep::exec (scheme_tree t) {
  ...
  if (is_atomic (t)) return t;
    ...
    if (is_tuple (t, "ypos", 4) && t[4] == "-")
      dy= probe (gl, xx, yy, 0, -1) * PIXEL;
    double r;
    if (is_tuple (t, "xpos")) r= ((double) (x + dx)) / hunit;
    else r= ((double) (y + dy)) / vunit;
    return as_string (r);
  }
...
virtual_font_rep::supported (scheme_tree t, bool svg) {
  ...
  if (is_tuple (t, "or") && N (t) >= 2) {
    int i, n= N (t);
    for (i= 1; i < n; i++)
      if (supported (t[i], svg)) return true;
    return false;
  }

  if (is_tuple (t, "join") || (is_tuple (t, "intersect", 2) && !svg) ||
      (is_tuple (t, "exclude", 2) && !svg) ||
      (is_tuple (t, "bitmap", 1) && !svg) || is_tuple (t, "glue", 2) ||
      is_tuple (t, "glue*", 2) || is_tuple (t, "glue-above", 2) ||
      is_tuple (t, "glue-below", 2) || is_tuple (t, "row", 2) ||
      is_tuple (t, "stack", 2) || is_tuple (t, "stack-equal", 2) ||
      is_tuple (t, "stack-less", 2) || is_tuple (t, "add", 2) ||
  ...
  if (is_tuple (t, "glue-above", 3) || is_tuple (t, "glue-below", 3) ||
      is_tuple (t, "stack", 3) ||
  ...
  if (is_tuple (t, "magnify", 3) || is_tuple (t, "deepen", 3) ||
      is_tuple (t, "widen", 3) || is_tuple (t, "enlarge") ||
      is_tuple (t, "unindent", 1) || is_tuple (t, "unindent*", 1) ||
      is_tuple (t, "crop", 1) || is_tuple (t, "hor-crop", 1) ||
      is_tuple (t, "left-crop", 1) || is_tuple (t, "right-crop", 1) ||
  ...
  if ((is_tuple (t, "align") && N (t) >= 3) ||
      (is_tuple (t, "align*") && N (t) >= 3) || is_tuple (t, "scale", 4) ||
      is_tuple (t, "scale*", 4) || (is_tuple (t, "fscale", 5) && !svg) ||
      (is_tuple (t, "fscale*", 5) && !svg) || is_tuple (t, "hor-scale", 2) ||
      is_tuple (t, "pretend", 2) || is_tuple (t, "hor-pretend", 2) ||
      is_tuple (t, "left-pretend", 2) || is_tuple (t, "right-pretend", 2) ||
      is_tuple (t, "ver-pretend", 2) || is_tuple (t, "reslash", 2) ||
      is_tuple (t, "negate", 2) || is_tuple (t, "min-width", 2) ||
      is_tuple (t, "max-width", 2) || is_tuple (t, "min-height", 2) ||
  ...
  if (is_tuple (t, "circle", 2) && !svg) return true;
...
virtual_font_rep::compile_bis (scheme_tree t, metric& ex) {
  ...
  if (is_tuple (t, "or") && N (t) >= 2) {
    int i, n= N (t);
    for (i= 1; i < n - 1; i++)
      if (supported (t[i], false)) return compile (t[i], ex);
  ...
  if (is_tuple (t, "bitmap", 1)) return compile (t[1], ex);
  ...
  if (is_tuple (t, "glue-above", 2) || is_tuple (t, "glue-above", 3)) {
    metric ey;
    glyph  gl1= compile (t[1], ex);
    glyph  gl2= compile (t[2], ey);
    SI     dy = ex->y2 - ey->y1;
    if (N (t) >= 4 && is_double (t[3])) dy+= (SI) (as_double (t[3]) * vunit);
    outer_fit (ex, ey, 0, dy);
    return join (gl1, move (gl2, 0, dy));
  }

  if (is_tuple (t, "glue-below", 2) || is_tuple (t, "glue-below", 3)) {
    metric ey;
    glyph  gl1= compile (t[1], ex);
    glyph  gl2= compile (t[2], ey);
    SI     dy = ex->y1 - ey->y2;
  ...
  if (is_tuple (t, "stack", 2) || is_tuple (t, "stack", 3)) {
    metric ey;
    glyph  gl1= compile (t[1], ex);
    glyph  gl2= compile (t[2], ey);
    SI     dy = ex->y1 - ey->y2;
    SI     up = (ey->y2 - ey->y1) >> 1;
    SI     lo = 0;
    if (N (t) >= 4 && is_double (t[3])) lo= (SI) (as_double (t[3]) * vunit);
    dy-= lo;
    up+= (lo >> 1);
    outer_fit (ex, ey, 0, dy);
    move (ex, 0, up);
  ...
  if (is_tuple (t, "stack-equal", 2)) {
    ...
    scheme_tree u=
        tuple ("align", tuple ("stack", t[1], t[2], d), "=", "*", "0.5");
    return compile (u, ex);
  }
  ...
  if (is_tuple (t, "stack-less", 2)) {
    scheme_tree d= exec (tuple ("sep-equal"));
    scheme_tree u=
        tuple ("align", tuple ("stack", t[1], t[2], d), "less", "*", "0.5");
    return compile (u, ex);
  }
  ...
  if (is_tuple (t, "left-fit", 3) && is_double (t[3]))
    return compile (
        tuple ("hor-flip", tuple ("right-fit", tuple ("hor-flip", t[1]),
                                  tuple ("hor-flip", t[2]), t[3])),
        ex);
  ...
  if (is_tuple (t, "crop", 1) || is_tuple (t, "hor-crop", 1) ||
      is_tuple (t, "left-crop", 1) || is_tuple (t, "right-crop", 1) ||
      is_tuple (t, "ver-crop", 1) || is_tuple (t, "top-crop", 1) ||
    ...
    if (is_tuple (t, "crop") || is_tuple (t, "hor-crop") ||
        is_tuple (t, "left-crop"))
      ex->x1= ex->x3= x1;
    if (is_tuple (t, "crop") || is_tuple (t, "hor-crop") ||
        is_tuple (t, "right-crop"))
      ex->x2= ex->x4= x2;
    if (is_tuple (t, "crop") || is_tuple (t, "ver-crop") ||
  ...
  if (is_tuple (t, "ver-flip", 1)) return ver_flip (compile (t[1], ex));
  ...
  if (is_tuple (t, "pretend", 2) || is_tuple (t, "hor-pretend", 2) ||
      is_tuple (t, "left-pretend", 2) || is_tuple (t, "right-pretend", 2) ||
  ...
  if (is_tuple (t, "font") && N (t) >= 3) return compile (t[1], ex);

  if (is_tuple (t, "italic", 3)) return compile (t[1], ex);
...
virtual_font_rep::draw_tree (renderer ren, scheme_tree t, SI x, SI y) {
  ...
  if (is_tuple (t, "part")) {
    ...
    SI oy= ex->y1, gh= ex->y2 - ex->y1;
    if (N (t) > 2 && t[2] != "*")
      ex->x1= ex->x3= ox + (SI) (as_double (t[2]) * gw);
    if (N (t) > 3 && t[3] != "*")
      ex->x2= ex->x4= ox + (SI) (as_double (t[3]) * gw);
    if (N (t) > 4 && t[4] != "*")
      ex->y1= ex->y3= oy + (SI) (as_double (t[4]) * gh);
    if (N (t) > 5 && t[5] != "*")
      ex->y2= ex->y4= oy + (SI) (as_double (t[5]) * gh);
    SI dx= 0, dy= 0;
    if (N (t) > 6) dx= (SI) (as_double (t[6]) * gw);
    if (N (t) > 7) dy= (SI) (as_double (t[7]) * gh);
  ...
  if (is_tuple (t, "copy", 1)) draw_tree (ren, t[1], x, y);
...
void
virtual_font_rep::draw_clipped (renderer ren, scheme_tree t, SI x, SI y, SI x1,
                                SI y1, SI x2, SI y2) {
...
virtual_font_rep::get_char (string s, font_metric& cfnm, font_glyphs& cfng) {
  ...
  if (n == 1) {
    ...
    if (is_nil (cfng->get (0))) cfng->get (0)= compile (t, cfnm->get (0));
    return 0;
  }
}
...
virtual_font_rep::get_xpositions (string s, SI* xpos, SI xk) {
  if (extend && base_fn->supports (s)) {
    base_fn->get_xpositions (s, xpos, xk);
    return;
  }
  metric ex;
  get_extents (s, ex);
...
virtual_font_rep::draw_fixed (renderer ren, string s, SI x, SI y) {
  if (extend && base_fn->supports (s)) base_fn->draw_fixed (ren, s, x, y);
...
virtual_font_rep::get_glyph (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_glyph (s);
  font_metric cfnm;
  font_glyphs cfng;
  int         c= get_char (s, cfnm, cfng);
...
int
virtual_font_rep::index_glyph (string s, font_metric& cfnm, font_glyphs& cfng) {
  if (extend && base_fn->supports (s))
...
virtual_font_rep::get_left_slope (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_left_slope (s);
  tree t= get_tree (s);
  if (is_tuple (t, "italic", 3)) return as_double (t[2]);
  return font_rep::get_left_slope (s);
}

double
virtual_font_rep::get_right_slope (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_right_slope (s);
  tree t= get_tree (s);
  if (is_tuple (t, "italic", 3)) return as_double (t[2]);
...
virtual_font_rep::get_right_correction (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_right_correction (s);
  tree t= get_tree (s);
  if (is_tuple (t, "italic", 3)) return (SI) (as_double (t[3]) * hunit);
  return font_rep::get_right_correction (s);
}

SI
virtual_font_rep::get_lsub_correction (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_lsub_correction (s);
...
virtual_font_rep::get_lsup_correction (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_lsup_correction (s);
...
virtual_font_rep::get_rsub_correction (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_rsub_correction (s);
...
virtual_font_rep::get_rsup_correction (string s) {
  if (extend && base_fn->supports (s)) return base_fn->get_rsup_correction (s);
  SI r= get_right_correction (s);
