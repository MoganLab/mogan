...
public:
  complex_pencil_rep (pencil_kind k2, brush b, SI w2, pencil_cap c,
                      pencil_join j, double l)
...
make_pencil (brush br, SI w) {
  if (br->get_type () == brush_none) return tm_new<no_pencil_rep> ();
  else if (br->get_type () == brush_color)
    return tm_new<simple_pencil_rep> (br->get_color (), w);
  else
    return tm_new<complex_pencil_rep> (pencil_brush, br, w, cap_round,
                                       join_round, 2.0);
}
...
make_pencil (tree t, int alpha, SI w) {
  if (is_atomic (t))
    ...
    brush br (t, alpha);
    if (is_none (br->get_pattern_url ())) return make_pencil ("grey", alpha, w);
    return tm_new<complex_pencil_rep> (pencil_brush, br, w, cap_round,
                                       join_round, 2.0);
  }
}
...
make_pencil (brush br, SI w, pencil_cap c, pencil_join j, double l) {
  if (br->get_type () == brush_none) return tm_new<no_pencil_rep> ();
    ...
    return tm_new<complex_pencil_rep> (pencil_standard, br->get_color (), w, c,
                                       j, l);
  else return tm_new<complex_pencil_rep> (pencil_brush, br, w, c, j, l);
}
...
make_pencil (tree t, int a, SI w, pencil_cap c, pencil_join j, double l) {
  if (is_atomic (t))
    ...
    return tm_new<complex_pencil_rep> (pencil_brush, brush (t, a), w, c, j, l);
  }
}

pencil::pencil (bool b)
    : rep (b ? (pencil_rep*) tm_new<simple_pencil_rep> (black,
                                                        std_shrinkf * PIXEL)
             : (pencil_rep*) tm_new<no_pencil_rep> ()) {
  INC_COUNT (rep);
}

pencil::pencil (color c, SI w) : rep (tm_new<simple_pencil_rep> (c, w)) {
  INC_COUNT (rep);
}
pencil::pencil (brush br, SI w) : rep (make_pencil (br, w)) { INC_COUNT (rep); }
pencil::pencil (tree t, int alpha, SI w) : rep (make_pencil (t, alpha, w)) {
  INC_COUNT (rep);
}

pencil::pencil (color col, SI w, pencil_cap c, pencil_join j, double l)
    : rep (tm_new<complex_pencil_rep> (pencil_standard, col, w, c, j, l)) {
  INC_COUNT (rep);
}
pencil::pencil (brush br, SI w, pencil_cap c, pencil_join j, double l)
    : rep (make_pencil (br, w, c, j, l)) {
  INC_COUNT (rep);
}
pencil::pencil (tree t, int a, SI w, pencil_cap c, pencil_join j, double l)
    : rep (make_pencil (t, a, w, c, j, l)) {
  INC_COUNT (rep);
}
