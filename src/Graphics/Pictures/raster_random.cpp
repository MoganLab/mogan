...
struct perlin_rep : rep<perlin> {
  ...
  double turbulence (int nColorChannel, double pointX, double pointY,
                     double fBaseFreqX, double fBaseFreqY, int nNumOctaves,
...
perlin_rep::perlin_rep (string name, long lSeed) : rep<perlin> (name) {
  ...
  for (k= 0; k < 4; k++) {
    for (i= 0; i < BSize; i++) {
      ...
      for (j= 0; j < 2; j++)
        fGradient[k][i][j]=
            (double) (((lSeed= random (lSeed)) % (BSize + BSize)) - BSize) /
            BSize;
...
double
perlin_rep::noise2 (int nColorChannel, double vec[2], StitchInfo* pStitchInfo) {
...
double
perlin_rep::turbulence (int nColorChannel, double pointX, double pointY,
                        double fBaseFreqX, double fBaseFreqY, int nNumOctaves,
                        bool bFractalSum, bool bDoStitching, double fTileX,
                        double fTileY, double fTileWidth, double fTileHeight) {
  StitchInfo  stitch;
  ...
  if (bDoStitching) {
    ...
    if (fBaseFreqX != 0.0) {
      ...
      if (fBaseFreqX / fLoFreq < fHiFreq / fBaseFreqX) fBaseFreqX= fLoFreq;
      else fBaseFreqX= fHiFreq;
    }
    if (fBaseFreqY != 0.0) {
      ...
      if (fBaseFreqY / fLoFreq < fHiFreq / fBaseFreqY) fBaseFreqY= fLoFreq;
      else fBaseFreqY= fHiFreq;
    }
...
raster<double>
turbulence (int w, int h, int ox, int oy, long seed, double wavelen_x,
            double wavelen_y, int nNumOctaves, bool bFractalSum) {
  perlin         p= perlin_generator (seed);
  raster<double> ret (w, h, ox, oy);
  for (int y= 0; y < h; y++)
    for (int x= 0; x < w; x++) {
      double v= p->turbulence (3, (double) x, (double) y, 1.0 / wavelen_x,
                               1.0 / wavelen_y, nNumOctaves, bFractalSum,
                               seed < 0, 0.0, 0.0, wavelen_x, wavelen_y);
...
raster<double>
turbulence (raster<double> ras, long seed, double wavelen_x, double wavelen_y,
            int nNumOctaves, bool bFractalSum) {
  return turbulence (ras->w, ras->h, ras->ox, ras->oy, seed, wavelen_x,
                     wavelen_y, nNumOctaves, bFractalSum);
}

raster<true_color>
turbulence (raster<true_color> ras, long seed, double wavelen_x,
            double wavelen_y, int nNumOctaves, bool bFractalSum) {
  ...
  for (int y= 0; y < h; y++)
    for (int x= 0; x < w; x++) {
      ...
      for (int ch= 0; ch < 4; ch++) {
        v[ch]= p->turbulence (ch, (double) x, (double) y, 1.0 / wavelen_x,
                              1.0 / wavelen_y, nNumOctaves, bFractalSum,
                              seed < 0, 0.0, 0.0, wavelen_x, wavelen_y);
