...
socket_link_rep::start () {
  ...
  if (DEBUG_AUTO) debug_io << "Connecting to '" << host << ":" << port << "'\n";
  ...
  */
  ...
  for (rp= result; rp != NULL; rp= rp->ai_next) {
    ...
    if (connect (io, rp->ai_addr, rp->ai_addrlen) != -1) break; /* Success */
...
socket_link_rep::feed (int channel) {
  ...
  if (r <= 0) {
    if (r == 0) debug_io << host << ":" << port << "' hung up\n";
      ...
      io_warning << "TeXmacs] read failed from '" << host << ":" << port
                 << "'\n";
    stop ();
  }
  else if (r != 0) {
...
socket_callback (void* obj, void* info) {
  ...
  if (!is_nil (con->feed_cmd) && news) con->feed_cmd->apply ();
}
#endif
