...
connection_start (string name, string session, bool again) {
  ...
  if (is_nil (con)) {
    ...
    if (is_tuple (t, "pipe", 1)) {
      ...
      if (is_nil (ln))
        return "Error: pipe connections not supported on this platform";
      ...
      if (is_nil (ln))
        return "Error: socket connections not supported on this platform";
      con= tm_new<connection_rep> (name, session, ln);
    }
#endif
      ...
      if (is_nil (ln))
        return "Error: dynamic link connections not supported on this platform";
      con= tm_new<connection_rep> (name, session, ln);
    }
  }
