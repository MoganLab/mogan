...
socket_server_rep::socket_server_rep (int port2) : port (port2), sn () {
...
#else
  if (setsockopt (server, SOL_SOCKET, SO_REUSEADDR, (const char*) &yes,
                  sizeof (int)) == -1)
#endif
...
socket_server_rep::start_client () {
  ...
  if (client == -1) io_warning << "Call to 'accept' failed\n";
    ...
    for (int i= 0; i < N (incoming); i++)
      if (incoming[i]->alive) update << incoming[i];
    incoming      = update;
