...
notify_graphics_extents (tree t, point lbot, point rtop) {
  ...
  if (is_atomic (t)) {
    ...
    if (is_func (t, LOCUS, 2) && is_func (t[0], ID, 1) && is_atomic (t[0][0]) &&
...
concater_rep::typeset_graphics (tree t, path ip) {
  ...
  bs << grid_box (ip, gr, env->fr, env->as_length ("2ln"), env->clip_lim1,
                  env->clip_lim2);
  ...
  if (crop && lim1[1] * lim2[1] > 0) {
    SI y1= env->fr (lim1)[1];
    SI y2= env->fr (lim2)[1];
    if (y1 > 0 && y2 > 0) b= move_box (ip, b, 0, -min (y1, y2));
    else if (y1 < 0 && y2 < 0) b= move_box (ip, b, 0, -max (y1, y2));
  }
  print (b);
...
is_transformation (tree t) {
  ...
  if (is_tuple (t, "scaling", 2) && is_double (t[1]) && is_double (t[2]))
    return true;
  if (is_tuple (t, "slanting", 1) && is_double (t[1])) return true;
  if (is_tuple (t, "linear", 4) && is_double (t[1]) && is_double (t[2]) &&
      is_double (t[3]) && is_double (t[4]))
    return true;
  return false;
}
...
concater_rep::typeset_text_at (tree t, path ip) {
  ...
  if (N (t) != 2) typeset_error (t, ip);
    ...
    if (N (p) == 0) typeset_dynamic (tree (ERROR, "bad text-at"), ip);
    else {
...
concater_rep::typeset_math_at (tree t, path ip) {
  ...
  if (N (t) != 2) typeset_error (t, ip);
    ...
    if (N (p) == 0) typeset_dynamic (tree (ERROR, "bad math-at"), ip);
    else {
      SI ox= (SI) p[0], oy= (SI) p[1], axis= (b->h () >> 1), x= ox, y= oy;
      if (halign == "left") x-= b->x1;
      else if (halign == "center") x-= ((b->x1 + b->x2) >> 1);
...
concater_rep::typeset_document_at (tree t, path ip) {
  ...
  if (N (t) != 2) typeset_error (t, ip);
    ...
    if (N (p) == 0) typeset_dynamic (tree (ERROR, "bad document-at"), ip);
    else {
      SI ox= (SI) p[0], oy= (SI) p[1], axis= (b->h () >> 1), x= ox, y= oy;
      if (halign == "left") x-= b->x1;
      else if (halign == "center") x-= ((b->x1 + b->x2) >> 1);
...
concater_rep::typeset_point (tree t, path ip) {
  ...
  if (N (t) < 2) typeset_error (t, ip);
    ...
    print (point_box (ip, p, env->point_size, pen, env->fill_brush,
                      env->point_style));
  }
  END_MAGNIFY
}
...
protect_arrow (edit_env env, tree t) {
  (void) env;
  return tree (WITH, "arrow-begin", "none", "arrow-end", "none", "dash-style",
               "none", t);
}
...
first_intersection (curve c, point lb, point rt) {
  ...
  while (t0 + dt <= 0.999999 && inside (c (t0 + dt), lb, rt))
    t0+= dt;
...
last_intersection (curve c, point lb, point rt) {
  ...
  while (t1 - dt >= 0.000001 && inside (c (t1 - dt), lb, rt))
    t1-= dt;
...
concater_rep::typeset_spline (tree t, path ip, bool close) {
  ...
  if (N (a) == 0 || N (a[0]) == 0) typeset_error (t, ip);
    ...
    curve c=
        env->fr (N (a) >= 3 ? spline (a, cip, close) : poly_segment (a, cip));
    adjust_extremities (c, env->white_zones);
    print (curve_box (ip, c, env->line_portion, env->pen, env->dash_style,
                      env->dash_motif, env->dash_style_unit, env->fill_brush,
                      typeset_line_arrows (ip)));
...
concater_rep::typeset_calligraphy (tree t, path ip) {
  ...
  if (N (t) < 4 || N (t[N (t) - 1]) < 1) typeset_error (t, ip);
    ...
    else if (is_func (enhance, TUPLE, 2) && is_atomic (enhance[0]) &&
             is_atomic (enhance[1])) {
    ...
    if (method == "gaussian" || method == "default") {
      ...
      if (is_compound (t[2], "ink-meta") && N (t[2]) >= 2 &&
          is_double (t[2][1]))
    ...
    if (is_func (t, CALLIGRAPHY)) {
      ...
      cb        = curve_box (ip, c, 1.0, pencil (col, 0), array<bool> (),
                             array<point> (), PIXEL, brush (col), array<box> ());
    }
    else
      cb= curve_box (ip, c, env->line_portion, env->pen, env->dash_style,
                     env->dash_motif, env->dash_style_unit, env->fill_brush,
                     typeset_line_arrows (ip));
    print (cb);
...
set_graphical_value (tree var, tree val) {
  // cout << "Set " << var << " := " << val << "\n";
  if (is_atomic (var)) graphical_values (var->label)= val;
}
...
graphics_require_update (tree var) {
  if (is_atomic (var)) graphical_modified->insert (var->label);
  // cout << "Set " << var << ", " << N(graphical_modified) << "\n";
}

void
graphics_notify_update (tree var) {
  if (is_atomic (var)) graphical_modified->remove (var->label);
...
concater_rep::typeset_graphics_3d (tree t, path ip) {
  ...
  if (is_nil (obj)) typeset_dynamic (tree (ERROR, "bad spacial object"), ip);
  else print (spacial_box (ip, transformed (obj, vt)));
  END_MAGNIFY
}
