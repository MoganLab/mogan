...
test_script_border (path p, box sb) {
  return is_accessible (sb->ip) && (path_up (p) == path_up (reverse (sb->ip)));
}
...
struct side_box_rep : public composite_box_rep {
  ...
  double left_slope () { return bs[id_left]->left_slope (); }
  double right_slope () { return bs[id_right]->right_slope (); }
  SI     left_correction () {
...
side_box_rep::side_box_rep (path ip, box ref, box l1, box l2, box r1, box r2,
                            font fn2, int level2)
...
side_box_rep::expand_glyphs (int mode, double factor) {
  ...
  box body= bs[0]->expand_glyphs (0, factor);
  box lsub, lsup, rsub, rsup;
  if ((type & 1) != 0) lsub= bs[1]->expand_glyphs (0, factor);
  if ((type & 2) != 0) lsup= bs[nr_left]->expand_glyphs (0, factor);
  if ((type & 4) != 0) rsub= bs[1 + nr_left]->expand_glyphs (0, factor);
  if ((type & 8) != 0) rsup= bs[N (bs) - 1]->expand_glyphs (0, factor);
  return side_box (ip, body, lsub, lsup, rsub, rsup, fn, level);
}
...
side_box_rep::find_tree_path (path bp) {
  if (is_atom (bp)) {
    ...
    case 1:
      ...
      if (nr_right == 1)
        return reverse (descend_script (bs[nr_left + 1]->ip, 1));
      return reverse (
          descend_script (bs[nr_left + 1]->ip, bs[nr_left + 2]->ip, 1));
    case 2:
      if (nr_left == 1) return reverse (descend_script (bs[1]->ip, 1));
      return reverse (descend_script (bs[1]->ip, bs[2]->ip, 1));
    case 3:
      if (nr_right == 1)
        return reverse (descend_script (bs[nr_left + 1]->ip, 0));
      return reverse (
          descend_script (bs[nr_left + 1]->ip, bs[nr_left + 2]->ip, 0));
    }
    TM_FAILED ("bad leaf");
