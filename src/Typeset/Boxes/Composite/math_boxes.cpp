...
inline void
left_italic_correct (box b) {
  b->x1-= b->left_correction ();
}
inline void
left_italic_restore (box b) {
  b->x1+= b->left_correction ();
}
inline void
right_italic_correct (box b) {
  b->x2+= b->right_correction ();
}
inline void
right_italic_restore (box b) {
  b->x2-= b->right_correction ();
}
...
sqrt_box_rep::sqrt_box_rep (path ip, box b1, box b2, box sqrtb, font fn2,
                            pencil pen2)
  ...
  SI sep  = fn->sep;
  SI wline= fn->wline;
  SI dx   = -fn->wfn / 20 - (sqrtb->x2 - sqrtb->x4),
...
sqrt_box_rep::find_child (SI x, SI y, SI delta, bool force) {
  (void) y;
  if (outside (x, delta, x1, x2) && (is_accessible (ip) || force)) return -1;
  int i= ((N (bs) == 3) || (x > sx (0)) || ((x >= sx (0)) && (delta >= 0))) ? 0
                                                                            : 1;
  if (bs[i]->decoration () && (!force)) {
    i= 1 - i;
    if (bs[i]->decoration () && (!force)) return -1;
...
neg_box_rep::neg_box_rep (path ip, box b, font fn2, pencil pen2)
  ...
  insert (line_box (decorate_middle (ip), X + DX, Y + DY, X - DX, Y - DY, npen),
          0, 0);
...
bool
compute_wide_accent (path ip, box b, string s, font fn, pencil pen,
                     bool request_wide, bool above, box& wideb, SI& sep) {
  ...
  if (wide) {
    if (tex_gyre) {
      if (s == "^" || s == "<hat>" || s == "~" || s == "<tilde>" ||
        ...
        if (starts (s, "<sq")) very_wide= (b->w () >= ((11 * fn->wfn) >> 2));
  ...
  if (wide && stix) {
    ...
    if (s == "<hat>" || s == "<tilde>" || s == "<check>" ||
      ...
      wideb=
          wide_stix_box (decorate_middle (ip),
                         "<rubber-" * s (1, N (s) - 1) * ">", rfn, pen, width);
      if (wideb->w () >= width) {
        if (b->right_slope () != 0)
  ...
  if (very_wide) {
    ...
    if ((s == "^") || (s == "<hat>"))
      ...
      wideb=
          wide_box (decorate_middle (ip), "<rubber-" * s (1, N (s) - 1) * ">",
                    fn, pen, b->x2 - b->x1);
    sep= fn->sep;
    ...
    wideb= resize_box (decorate_middle (ip), wideb, max (wideb->x1, wideb->x3),
                       wideb->y1, min (wideb->x2, wideb->x4), wideb->y2);
...
struct wide_box_rep : public composite_box_rep {
  ...
  SI left_correction () { return ref->left_correction (); }
  SI right_correction () {
    /*
    if (above) {
  ...
  SI lsub_correction () { return ref->lsub_correction (); }
  SI lsup_correction () { return ref->lsup_correction (); }
  SI rsub_correction () { return ref->rsub_correction (); }
  SI rsup_correction () {
    /*
    if (above) {
  ...
  SI   wide_correction (int mode) { return ref->wide_correction (mode); }
  void get_bracket_extents (SI& lo, SI& hi);
};

wide_box_rep::wide_box_rep (path ip, box ref2, string s2, font fn2, pencil pen2,
