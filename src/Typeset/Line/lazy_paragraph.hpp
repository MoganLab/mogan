...
protected:
  ...
  void handle_decorations (int& i, int& j, SI& xoff, array<box>& bs,
                           array<SI>& bs_sp);
  ...
  void line_unit (path start, path end, bool break_flag, string mode,
                  SI the_left, SI the_right);
  void line_end (space spc, int penalty);
  void line_units (int start, int end, bool is_start, bool is_end, string mode,
                   string hyphen, SI the_left, SI the_right, SI the_first,
                   SI the_last);
...
public:
  ...
  friend array<page_item> typeset_stack (edit_env env, tree t, path ip,
                                         SI width, array<line_item> a,
...
struct lazy_paragraph {
  ...
  inline lazy_paragraph (edit_env env, path ip)
      : rep (tm_new<lazy_paragraph_rep> (env, ip)) {
    rep->ref_count= 1;
  }
};
