...
edit_env_rep::update_page_pars () {
  ...
  if (page_automatic) {
    ...
    if (page_type == "user") {
      if (get_string (PAGE_EVEN) == "auto" && get_string (PAGE_ODD) != "auto")
        page_even_margin= page_odd_margin;
      if (get_string (PAGE_ODD) == "auto" && get_string (PAGE_EVEN) != "auto")
...
void
edit_env_rep::get_page_pars (SI& w, SI& h, SI& width, SI& height, SI& odd,
                             SI& even, SI& top, SI& bot) {
  w     = page_user_width;
  h     = page_user_height;
  width = page_width;
  height= page_height;
...
edit_env_rep::get_page_width (bool deco) {
  ...
  if (get_string (PAGE_MEDIUM) == "paper" &&
      get_string (PAGE_BORDER) != "none" && deco)
    w+= 20 * pixel;
  return w;
}
...
edit_env_rep::get_pages_width (bool deco) {
  ...
  w= w * page_packet;
  if (get_string (PAGE_MEDIUM) == "paper" &&
      get_string (PAGE_BORDER) == "attached" && deco)
    w+= 20 * pixel;
  return w;
}
...
edit_env_rep::get_ornament_parameters () {
  ...
  SI lw, bw, rw, tw;
  if (is_atomic (w) && occurs (",", w->label)) w= tuplify (w);
  ...
  double lx, bx, rx, tx;
  if (is_atomic (ext) && occurs (",", ext->label)) ext= tuplify (ext);
  ...
  SI lpad, rpad;
  if (is_atomic (xpad) && occurs (",", xpad->label)) xpad= tuplify (xpad);
  ...
  SI bpad, tpad;
  if (is_atomic (ypad) && occurs (",", ypad->label)) ypad= tuplify (ypad);
  ...
  if (is_percentage (cor)) {
    ...
    if (is_atomic (cor)) cor= tuplify (cor);
    if (is_func (cor, TUPLE, 2)) cor= tuple (cor[0], cor[1], cor[0], cor[1]);
    if (!is_func (cor, TUPLE, 4)) lcor= bcor= rcor= tcor= 0;
    else {
  ...
  return ornament_parameters (shape, tst, lw, bw, rw, tw, lx, bx, rx, tx, lpad,
                              bpad, rpad, tpad, lcor, bcor, rcor, tcor,
  ...
  for (int i= 1; i < N (t); i++)
    if (is_func (t[i], TUPLE) && N (t[i]) >= 2) {
      ...
      for (int j= 0; j + 1 < N (u); j+= 2) {
        tree var= exec (u[j]);
        tree val= exec (u[j + 1]);
        if (is_atomic (val) && is_length (val->label)) val= as_tmlen (val);
        r << var << val;
        if (main_text) {
  ...
  if (is_tuple (szt))
    for (int i= 0; i < N (szt); i++)
      if (is_tuple (szt[i]) && N (szt[i]) >= 1)
        if (szt[i][0] == "all" || szt[i][0] == as_string (sz)) {
...
decode_alpha (string s) {
  if (N (s) == 0) return 255;
    ...
    // mg: be careful to avoid rounding problems for the conversion from double
    // to int : (int)(2.55*100.0)=254
...
edit_env_rep::update_geometry () {
  ...
  if (is_tuple (t, "geometry", 2) || is_tuple (t, "geometry", 3)) {
    if (is_length (as_string (t[1])) || is_anylen (t[1])) gw= as_length (t[1]);
    if (is_length (as_string (t[2])) || is_anylen (t[2])) gh= as_length (t[2]);
    if (is_tuple (t, "geometry", 3)) gvalign= as_string (t[3]);
  }
  update_frame ();
}
...
edit_env_rep::update_dash_style () {
  ...
  if (is_string (t)) {
    ...
    if (N (s) > 0 && (s[0] == '0' || s[0] == '1')) {
      ...
      if (s == "zigzag") dash_motif << point (0.25, 0.5) << point (0.75, -0.5);
      else if (s == "wave")
        for (int k= 1; k <= 11; k++)
          dash_motif << point (k / 12.0, sin (3.141592 * k / 6.0) / 2.0);
      else if (s == "pulse")
        dash_motif << point (0.0, 0.5) << point (0.5, 0.5) << point (0.5, -0.5)
                   << point (1.0, -0.5);
      else if (s == "loops")
        for (int k= 1; k <= 11; k++) {
        ...
        double u= 1.0 / 12.0;
        dash_motif << point (-0.5 * u, 0.0) << point (5 * u, 0.0)
                   << point (5 * u, 5 * u) << point (u, 5 * u)
                   << point (u, 2 * u) << point (3 * u, 2 * u)
                   << point (3 * u, 3 * u) << point (2 * u, 3 * u)
                   << point (2 * u, 4 * u) << point (4 * u, 4 * u)
                   << point (4 * u, u) << point (0.0, u) << point (0.0, 6 * u)
                   << point (5.5 * u, 6 * u) << point (11 * u, 6 * u)
                   << point (11 * u, u) << point (7 * u, u)
                   << point (7 * u, 4 * u) << point (9 * u, 4 * u)
                   << point (9 * u, 3 * u) << point (8 * u, 3 * u)
                   << point (8 * u, 2 * u) << point (10 * u, 2 * u)
                   << point (10 * u, 5 * u) << point (6 * u, 5 * u)
                   << point (6 * u, 0.0);
...
edit_env_rep::update (string s) {
  ...
  case Env_Math_Width:
    ...
    table_max= get_length (MATH_TABLE_LIMIT);
    flatten_pen=
        pencil (env[MATH_FLATTEN_COLOR], alpha, get_length (LINE_WIDTH));
    break;
  case Env_Color:
    update_color ();
    break;
