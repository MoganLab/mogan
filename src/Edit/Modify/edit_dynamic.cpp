...
edit_dynamic_rep::in_source () {
  ...
  // get a correct value.
  if (!env_locked) env_in_source= get_env_string (MODE) == "src";
...
edit_dynamic_rep::is_multi_paragraph_macro (tree t, bool pure) {
  ...
  if (is_extension (t) && !is_compound (t, "footnote") &&
    ...
    int i;
    for (i= 1; i < n; i++)
      if (is_multi_paragraph_macro (t[i], pure)) return true;
    tree f= get_env_value (as_string (L (t)));
    return is_multi_paragraph_macro (f, pure);
  }
  if (!pure)
    if (is_func (t, ARG)) return true;
  return false;
}
...
contains_table_format (tree t, tree var) {
  ...
  if (is_atomic (t)) return false;
    ...
    int i, n= N (t);
    for (i= 0; i < n; i++)
      if (contains_table_format (t[i], var)) return true;
...
edit_dynamic_rep::make_compound (tree_label l, int n= -1) {
  ...
  if (n == -1) {
    for (n= 0; true; n++) {
      if (drd->correct_arity (l, n) &&
          ((n > 0) || (drd->get_arity_mode (l) == ARITY_NORMAL)))
        break;
      if (n == 100) return;
    }
  }
  ...
  int  acc= 0;
  for (; acc < n; acc++)
    if (drd->is_accessible_child (t, acc)) break;
  if (acc < n) p->item= acc;
  if (n == 0) insert_tree (t, 1);
    ...
    if ((block_macro && (!table_macro) && l != INCLUDE && l != VAR_INCLUDE) ||
...
edit_dynamic_rep::go_to_argument (path p, bool start_flag) {
  ...
  if (i < 0) go_to_start (path_up (p, inactive ? 2 : 1));
    ...
    if ((!drd->is_accessible_child (t, i)) && (!inactive) && (!in_source ())) {
...
edit_dynamic_rep::insert_argument (path p, bool forward) {
  ...
  if ((!in_source ()) || drd->contains (as_string (L (t)))) {
    if (forward) do
        i++;
      while ((i <= n) && (!drd->insert_point (L (t), i, n)));
    else
      while ((i >= 0) && (!drd->insert_point (L (t), i, n)))
        i--;
    if ((i < 0) || (i > n)) return;
    while (!drd->correct_arity (L (t), n + d))
      d++;
  }
  else if (forward) i++;
  path q= path_up (p) * i;
...
edit_dynamic_rep::remove_empty_argument (path p, bool forward) {
  ...
  for (d= 1; d <= n - i; d++)
    if ((src_flag && (d == 1)) ||
        ((!src_flag) && drd->correct_arity (L (t), n - d) &&
         drd->insert_point (L (t), i, n - d))) {
      bool flag= true;
      for (j= 0; j < d; j++)
        flag= flag && is_empty (t[i + j]);
      if (flag) {
        bool old_locked= env_locked;
        env_locked     = true;
        remove (p, d);
  ...
  if ((!in_source ()) || drd->contains (as_string (L (t)))) {
    if (forward) do
      ...
      while (i >= 0 && !drd->insert_point (L (t), i, n))
        i--;
    if ((i < 0) || (i > n)) return;
    while (i >= d && !drd->correct_arity (L (t), n - d))
      d++;
...
edit_dynamic_rep::make_hybrid () {
  tree t (HYBRID, "");
  if (selection_active_small ()) t[0]= selection_get_cut ();
  ...
  set_message (concat (kbd ("return"), ": activate symbol or macro"), "hybrid");
}
...
edit_dynamic_rep::activate_latex () {
  ...
  tree st= subtree (et, p);
  if (is_atomic (st[0])) {
    if (is_func (subtree (et, path_up (p)), INACTIVE)) p= path_up (p);
...
edit_dynamic_rep::activate_hybrid (bool with_args_hint) {
  ...
  if ((drd->contains (name) && (f == UNINIT)) || is_func (f, MACRO) ||
      is_func (f, XMACRO)) {
    assign (p, "");
    correct (path_up (p));
...
edit_dynamic_rep::activate_symbol () {
  ...
  if (is_func (subtree (et, path_up (p)), INACTIVE)) p= path_up (p);
  string s= st[0]->label;
  if (is_int (s)) assign (p, string (((char) as_int (s))));
  else {
    int i, n= N (s);
    for (i= 0; i < n; i++)
      if ((s[i] == '<') || (s[i] == '>')) {
