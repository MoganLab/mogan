...
edit_text_rep::pure_line (path p) {
  ...
  return is_document (st) ||
         ((is_func (st, WITH) || is_func (st, LOCUS) || is_func (st, CANVAS) ||
...
edit_text_rep::insert_return () {
  if (accepts_return (path_up (tp))) insert_node (path_up (tp) * 0, CONCAT);
  path p= path_up (tp, 2);
...
edit_text_rep::remove_return (path p) {
  ...
  if (!is_concat (subtree (et, p))) insert_node (p * 0, CONCAT);
...
edit_text_rep::prepare_for_insert () {
  ...
  if ((!is_document (st)) && is_multi_paragraph (st)) {
    if (!is_document (subtree (et, path_up (p)))) insert_node (p * 0, DOCUMENT);
    else {
      if (l == 0) {
...
edit_text_rep::insert_tree (tree t, path p_in_t) {
  ...
  if (is_atomic (t) && (p_in_t == end (t)) &&
    ...
    if (subtree (et, path_up (tp)) == "" && accepts_return (path_up (tp)) &&
...
edit_text_rep::var_insert_tree (tree t, path p_in_t) {
  if (selection_active_any ()) {
    selection_cut ("wrapbuf");
    insert_tree (t, p_in_t);
    selection_paste ("wrapbuf");
  }
...
edit_text_rep::make_image (string file_name, bool link, string w, string h,
  ...
  if (link) {
    ...
    // here we convert the filename to cork-universal because it is exposed in
    // the document, in the <image> tag
