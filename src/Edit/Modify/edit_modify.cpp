...
local_update_cb (void* user_data, const uint8_t* bytes, size_t len) {
  if (g_loro_broadcast_update) {
    cout << "[Loro] Local update generated, size: " << len
         << " bytes. Broadcasting...\n";
    string data ((const char*) bytes, len);
    g_loro_broadcast_update (data);
  }
  else {
    cout << "[Loro] Local update generated, but no broadcast handler "
            "registered.\n";
  }
}
#endif
...
// - 文本精确路径：mirror 后 Loro 已含 op → diff 回译出 mod → 应用时跳过
// mirror_loro（防重复）。
// - 结构兜底：mirror 未改变 Loro（重 seed 自 before）→ diff 为空 → 正常应用原
// mod
...
edit_modify_rep::route_through_loro (const modification& mod) {
  ...
  if (is_nil (mods)) {
    edit_announce (
        this, mod); // 结构兜底：正常应用（mirror_loro 重 seed 自 post-edit）
  }
  else {
    loro_applying_remote=
        true; // 文本精确：Loro 已有 op，应用回译 mod 时跳过 mirror_loro
    for (list<modification> l= mods; !is_nil (l); l= l->next)
...
edit_announce (editor_rep* ed, modification mod) {
  // NOTE: 曾尝试在此拦截做 debug_loro round-trip，但 edit_observer 对每个 mod
  // 配对调用 edit_announce(应用)+edit_done(mirror)，在 announce 里 mirror
  // 后外层 done 会重复 mirror （文本路径插两次）。盲写难以安全区分内/外层
  // done，故暂不激活。debug_loro 改用验证模式。
#ifdef LIII_DEBUG
