...
can_snap (gr_selection sel) {
  ...
  if (type == "point") return check_snap_mode ("control point");
  if (type == "curve-handle") return check_snap_mode ("control point");
  if (type == "curve-point") return check_snap_mode ("curve point");
  if (type == "curve-point&curve-point")
    return check_snap_mode ("curve-curve intersection");
  if (type == "grid-point") return check_snap_mode ("grid point");
  if (type == "grid-curve-point") return check_snap_mode ("grid curve point");
  if (type == "curve-point&grid-curve-point")
    return check_snap_mode ("curve-grid intersection");
  if (type == "grid-curve-point&curve-point")
    return check_snap_mode ("curve-grid intersection");
  if (type == "text" || type == "group") return check_snap_mode ("text");
  if (type == "text-handle") return check_snap_mode ("control point");
  if (type == "text-border") return check_snap_mode ("text border");
  if (type == "text-border-point") return check_snap_mode ("text border point");
...
snap_to_guide (point p, gr_selections sels, double eps) {
  ...
  for (int i= 0; i < N (sels); i++)
    if (can_snap (sels[i])) {
      if (sels[i]->type == "grid-point") best= sels[i];
      else if (is_nil (sels[i]->c) && best->type != "grid-point" &&
  ...
  for (int i= 0; i < N (sels); i++)
    for (int j= i + 1; j < N (sels); j++) {
      if (!is_nil (sels[i]->c) && !is_nil (sels[j]->c) &&
...
edit_graphics_rep::adjust (point p) {
  ...
  if ((tree) g != "empty_grid") {
    ...
    array<grid_curve> gc= g->get_curves_around (p, snap_distance, f);
...
edit_graphics_rep::graphical_select (double x, double y) {
  ...
  for (int i= 0; i < N (pre_sels); i++)
    if (admissible_selection (pre_sels[i])) sels << pre_sels[i];
  // for (int i=0; i<N(sels); i++)
  //   cout << i << ":\t" << sels[i] << "\n";
  gs      = sels;
  gr0     = empty_grid ();
...
tree
edit_graphics_rep::graphical_select (double x1, double y1, double x2,
                                     double y2) {
  frame f= find_frame ();
  if (is_nil (f)) return tuple ();
  gr_selections sels;
...
edit_graphics_rep::invalidate_graphical_object () {
  SI gx1, gy1, gx2, gy2;
  if (!is_nil (eb) && find_graphical_region (gx1, gy1, gx2, gy2) &&
      !is_nil (go_box)) {
    int        i;
    rectangles rs;
...
edit_graphics_rep::draw_graphical_object (renderer ren) {
  ...
  for (i= 0; i < go_box->subnr (); i++) {
    box b= go_box->subbox (i);
    if ((tree) b == "point" || (tree) b == "curve") b->display (ren);
    else {
      rectangles rs;
...
edit_graphics_rep::mouse_graphics (string type, SI x, SI y, int mods, time_t t,
  ...
  if (!is_nil (f)) {
    ...
    if (type == "move") call ("graphics-move", sx, sy);
    ...
    else if (type == "release-middle") call ("graphics-release-middle", sx, sy);
