    ...
    if (test (s, i, "{\\btm{") || test (s, i, "{\\etm{")) {
      ...
      if (skip_curly (s, i)) {
        ...
        if (!is_nil (p) && (l->contains (p) || N (l) == 0))
          if (i < n && s[i] == '}') {
            if (!corr->contains (N (r))) corr (N (r))= array<path> ();
            corr (N (r)) << (p * suffix);
            i++;
            continue;
          }
...
get_invalid_regions (string s1, int b1, int e1, string s2, int b2, int e2,
  ...
  while (b1 < e1 && b2 < e2 && s1[e1 - 1] == s2[e2 - 1]) {
    e1--;
    e2--;
  }
  int sb1, se1, sb2, se2;
...
void
latex_check_transparency (string ums, string s, hashmap<int, array<path>> corr,
...
tree_to_latex_document (tree d, object opts) {
  eval ("(use-modules (latex latex-format))");
...
purify (tree d) {
  ...
  tree r (DOCUMENT);
  r << compound ("TeXmacs", v) << compound ("style", s);
