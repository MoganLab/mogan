...
tex_rubber_font_rep::supports (string c) {
  ...
  return starts (c, "<large-") || starts (c, "<left-") || starts (c, "<mid-") ||
         starts (c, "<right-") || starts (c, "<big-");
}
...
tex_rubber_font_rep::get_partial_extents (int c, metric& ex) {
  ...
  ex->y3= min (ex->y3, ex->y1 + (ey->y3 - ey->y1));
  ex->x4= max (ex->x4, ey->x4);
  ex->y4= max (ex->y4, ex->y1 + (ey->y4 - ey->y1));
}
...
tex_rubber_font_rep::get_extents (string s, metric& ex) {
  ...
  if (tfm->tag (c) != 3) get_raw_extents (c, ex);
    ...
    if ((tfm->rep (c) != 0) && (tfm->mid (c) != 0))
      for (i= 0; i < nr_rep; i++)
        get_partial_extents (tfm->rep (c), ex);
