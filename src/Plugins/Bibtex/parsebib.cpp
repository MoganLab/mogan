...
bib_error () {
  ...
  else convert_error << "BibTeX parse error in " << bib_current_tag << "\n";
}
...
bib_char (string s, int& pos, char c) {
  ...
  if (s[pos] == c) pos++;
    ...
    bib_error ();
    if (c)
      convert_error << "Invalid char: \'" << s[pos] << "\', expected \'" << c
                    << "\'\n";
    return -1;
  }
  return 0;
...
bib_within (string s, int& pos, char cbegin, char cend, string& content) {
  if (!bib_ok (s, pos)) return;
  int depth= 0;
  if (bib_char (s, pos, cbegin)) return;
...
void
bib_atomic_arg (string s, int& pos, string ce, tree& a) {
  if (!bib_ok (s, pos)) return;
  string sa;
  string f, v, j, l;
  switch (s[pos]) {
...
bib_fields (string s, int& pos, string ce, string tag, tree& fields) {
  ...
  while (bib_ok (s, pos) && !bib_is_in (s[pos], ce)) {
    ...
    string cend= ce;
    cend << ",";
    while (bib_ok (s, pos) && !bib_is_in (s[pos], cend))
      pos++;
    while (bib_ok (s, pos) && s[pos] == ',') {
      pos++;
      bib_blank (s, pos);
...
bib_entry (string s, int& pos, tree type, tree& t) {
  ...
  entry << type << tree (utf8_to_cork (tag)) << fields;
  t << entry;
}
        ...
        if (stype == "string") {
          ...
          if (bibtex_non_empty_comment (s (start, pos + 1))) {
            ...
            for (int l= 0; l < N (lines); l++)
              if (trim_spaces (lines[l]) != "") doc << tree (lines[l]);
            if (N (doc) != 0) tentry << compound ("bib-comment", doc);
          }
          /* end */
          if (comment) {
