 ...
 *
 * @par 设计
 * - @b 只读数据：构造时读取 Scheme 最近文档并去重。
 *   路径转为 UTF-8；QML 只读候选模型和翻译文案。
 *   筛选和选择状态始终留在 QML 本地。
 * - @b 确认回流：open() 保存用户显式选择的路径并结束模态。
 *   closeBridge 处理取消、Esc 与窗口拖动。
 *
 * @note bridge 不挂 QObject parent，调用方在 exec 返回后删除。
 *       宿主 QDialog 只在 open() 期间借用。
 */
...
private:
   ...
   * @note url->system 已返回 UTF-8 系统路径，不能再转为 Cork。
   */
