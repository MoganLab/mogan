 ...
 * 段错误（QRhi::endOffscreenFrame）。取色期间隐藏宿主弹窗，结束后恢复。
 *
 * @note macOS 下抓屏需「屏幕录制」权限，未授权时 grabWindow 返回空图，本次
 * 取色返回空串（不可用），与 Qt 自带 QColorDialog 行为一致。Wayland 不允许
 * 应用任意抓屏（grabWindow 返回空图或全黑快照），canPickScreen 为 false，
 * QML 侧据此隐藏「Pick screen color」按钮。
 */
...
public:
  /**
   * @brief 构造桥对象。
   * @param host 宿主弹窗（取色期间临时隐藏，结束后恢复），不挂 parent。
   */
  explicit ColorPickerBridge (QWidget* host= nullptr, QObject* parent= nullptr);

  /** @brief 取色 overlay 未关闭时一并销毁，避免遗留全屏无父浮层。 */
  ~ColorPickerBridge () override;

  /** @brief 当前平台是否支持屏幕取色（Wayland 不支持）。 */
  bool canPickScreen () const;

  /**
   * @brief 发起屏幕取色：隐藏宿主弹窗，全屏快照 overlay 等待点击。
   *
   * 结果经 screenColorPicked 异步回流；取消（Esc）或平台不支持时携带空串。
   */
