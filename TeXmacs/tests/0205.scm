          ...
          (let ((sid-pair (assoc "sessionId" e)))
            (when (and sid-pair (== (cdr sid-pair) sid))
              (let ((field-pair (assoc field e)))
                (when field-pair (set! result (cdr field-pair)))
              ) ;let
            ) ;when
          ) ;let
        ) ;when
...
(define (test-chat-persist-manifest-path)
  (let ((base (chat-persist-base-dir)))
    (check (chat-persist-manifest-path)
      => (string-append base "/manifest.json"))
  ) ;let
) ;define
...
(define (test-chat-persist-ensure-dir!)
  (let ((nested (string-append "/tmp/chat-persist-test-"
                  (number->string (current-time)) "/a/b/c")))
    ;; 创建前不存在
...
(define (test-chat-persist-load-all-multi)
  (chat-persist-with-backup
    (lambda ()
      (let* ((sid1 (string-append "loadm1-"
                    (number->string (current-time))))
             (sid2 (string-append "loadm2-"
                    (number->string (current-time))))
             (model1 "ModelA")
             (model2 "ModelB")
            ) ;
...
(define (test-chat-persist-load-all-missing-message-file)
  (chat-persist-with-backup
    (lambda ()
      (let* ((sid (string-append "loadd-missing-"
                    (number->string (current-time))))
             (model "MissingModel")
...
(define (test-chat-persist-save-one-append)
  (chat-persist-with-backup
    (lambda ()
      (let* ((sid1 (string-append "so-app1-"
                    (number->string (current-time))))
             (sid2 (string-append "so-app2-"
                    (number->string (current-time))))
             (model1 "ModelA")
             (model2 "ModelB")
            ) ;
...
(define (test-chat-persist-save-one-update-preserves-others)
  (chat-persist-with-backup
    (lambda ()
      (let* ((sid1 (string-append "so-pr1-"
                    (number->string (current-time))))
             (sid2 (string-append "so-pr2-"
                    (number->string (current-time))))
             (model1 "ModelA")
             (model2 "ModelB")
            ) ;
...
(define (test-chat-persist-save-one-mixed)
  (chat-persist-with-backup
    (lambda ()
      (let* ((sid1 (string-append "so-mix1-"
                    (number->string (current-time))))
             (sid2 (string-append "so-mix2-"
                    (number->string (current-time))))
             (model1 "ModelA")
             (model2 "ModelB")
            ) ;
...
(define (test-chat-persist-save-one-three-sessions)
  (chat-persist-with-backup
    (lambda ()
      (let* ((sid1 (string-append "so-3a-"
                    (number->string (current-time))))
             (sid2 (string-append "so-3b-"
                    (number->string (current-time))))
             (sid3 (string-append "so-3c-"
                    (number->string (current-time))))
             (model "TriModel")
            ) ;
