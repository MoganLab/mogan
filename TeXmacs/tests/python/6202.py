...
Automated UI test and screenshot utility for issue 6202:
Verify unordered list rendering inside quote-env blockquote in AI sidebar / LLM chat.
Unordered lists in quote-env wrap correctly within the container without horizontal
clipping or text truncation at line breaks.

Usage:
  python3 6202.py [path/to/doc.tmu] [output_screenshot.png]
"""
...
def capture_quote_env(doc_path=None, output_path=None):
    ...
    if doc_path is None:
        default_candidates = [
            os.path.join(repo_root, "TeXmacs", "tests", "tmu", "6202.tmu"),
            os.path.join(repo_root, "6202.tmu"),
        ]
    ...
    if output_path is None:
        output_path = "/tmp/6202.png"

    print(f"[6202] Target document: {doc_path}")
    print(f"[6202] Output screenshot: {output_path}")
    print(f"[6202] Using Mogan binary: {bin_path}")

    env = os.environ.copy()
    env["TEXMACS_PATH"] = os.path.join(repo_root, "TeXmacs")

    print("[6202] Step 1: Launching Mogan STEM...")
    ...
    try:
        ...
        ret = proc.poll()
        if ret is not None:
            print(f"[6202] ERROR: Mogan exited prematurely with code {ret}")
            return 1

        print("[6202] Step 2: Focusing Mogan window...")
        focus_mogan_window()
        time.sleep(0.5)

        wx, wy, ww, wh = get_mogan_window_rect()
        print(f"[6202] Mogan window bounds: ({wx}, {wy}, {ww}, {wh})")
        ...
        click_x = wx + ww // 2
        click_y = wy + wh // 2
        print(f"[6202] Step 3: Using pynput to click document area at ({click_x}, {click_y})...")
        mouse.position = (click_x, click_y)
        time.sleep(0.3)
        mouse.click(Button.left)
        time.sleep(1.0)

        print("[6202] Step 4: Capturing screenshot...")
        ...
        print(f"[6202] Screenshot successfully saved to: {output_path}")

        print("[6202] Step 5: Using pynput to close Mogan (Ctrl+Q)...")
        ...
        print("[6202] Verification completed successfully.")
        return 0
    finally:
