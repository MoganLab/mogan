...
(define (collect-all-regions options num-rows)
  (let loop-r ((r 1) (regions '()))
    (if (> r num-rows)
        regions
        (let* ((r-str (number->string r))
               (row-regions
                (let loop-c ((c 1) (c-acc '()))
                  (if (> c 50)
                      c-acc
                      (let* ((c-str (number->string c))
                             (h-val (let loop-opt ((lst options))
                                      (cond ((null? lst) #f)
                                            ((and (pair? (car lst))
                                                  (eq? (caar lst) 'cwith)
                             ...
                             (w-val (let loop-opt ((lst options))
                                      (cond ((null? lst) #f)
                                            ((and (pair? (car lst))
                                                  (eq? (caar lst) 'cwith)
                                                  (equal? (list-ref (car lst) 1) r-str)
...
(define (clean-covered-cells-in-row cells r c regions new-cells-acc)
  (cond ((null? cells)
         (reverse new-cells-acc))
        (else
         (let* ((cell (car cells))
                (new-cell (if (is-cell-covered? r c regions)
                              '(cell "")
                              cell)))
           (clean-covered-cells-in-row (cdr cells) r (+ c 1) regions (cons new-cell new-cells-acc))))))

(define (clean-covered-cells-in-rows rows r regions new-rows-acc)
  (cond ((null? rows)
         (reverse new-rows-acc))
        (else
         (let* ((row (car rows))
                (cells (cdr row))
                (new-cells (clean-covered-cells-in-row cells r 1 regions '()))
                (new-row (cons 'row new-cells)))
           (clean-covered-cells-in-rows (cdr rows) (+ r 1) regions (cons new-row new-rows-acc))))))
