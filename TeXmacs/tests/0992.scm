...
;; 正文类场景（含 math 公式内）的期望值：企业版且「ai:actions bar」开关开启时
;; 才允许弹出；pref key 与 C++ 侧字面量一致，pref-keys.scm 的 accessor 在测试
;; 默认模块作用域不可见

(define (body-selection-expected)
  (and (not (community-stem?)) (== (get-preference "ai:actions bar") "on"))
) ;define
...
(tm-define (test_0992)
  (run-chain
    (list
      ...
      ;; 1) 正文选区：期望值随 flavor 与首选项计算。
      ;; body 树：(document (show-preamble (document "PREAMBLETEXT")) "BODYTEXT")
      (cons "body selection"
        (lambda ()
          (let ((body-expected (body-selection-expected)))
      ...
      (cons "math document" (lambda () (new-document) (insert '(math "xy"))))
      (cons "math selection assert"
        (lambda ()
      ...
      (cons "src-mode buffer selection"
        (lambda () (load-buffer "$TEXMACS_PATH/styles/article.ts"))
      ) ;cons
      ...
      (cons "image-only document"
        (lambda ()
          (new-document)
          (insert '(image "$TEXMACS_PATH/misc/images/new-mogan-128.png" "77pt"
                     "77pt" "" "")
          ) ;insert
        ) ;lambda
      ) ;cons
