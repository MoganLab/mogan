...
(use-modules (math math-edit)
             (generic semantic-popup))
...
(define (test_6006)
  ...
  (let* ((eq-star (stree->tree '(equation* (document "x+y=z"))))
    ...
    (check (caddr acts-star) => '(action "toggle-number" "Add Number" "tm_numbered")))
  ...
  (let* ((eq-num (stree->tree '(equation (document "x+y=z"))))
    ...
    (check (caddr acts-num) => '(action "toggle-number" "Hide Number" "tm_numbered")))
  ...
  (let* ((eq (stree->tree '(equation (document "a=b"))))
         (res (semantic-copy-latex eq)))
    (check (string? res) => #t)
  ...
  (let ((node (stree->tree '(equation* (document "1+1=2")))))
    (check (string? (semantic-popup-action-trigger 'equation* "copy-latex" node)) => #t)
    (check (semantic-popup-action-trigger 'equation* "toggle-number" node) => 'equation)
    (check (tree-label node) => 'equation))
