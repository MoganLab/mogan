  ...
  (cond ((null? args)
        ...
        ((== (length args) 1)
         (append
           ...
           (let ((result (ext-apply-on-paragraphs "numbered-line" (car args))))
             (if (tm-func? result 'document)
                 (tm-children result)
                 (list result)))
           (list `(numbered-line (concat (render-end-if) (right-flush))))))
        (else
         (let ((cond (car args))
           ...
           (append
             ...
             (let ((result (ext-apply-on-paragraphs "numbered-line" body)))
               (if (tm-func? result 'document)
                   (tm-children result)
                   (list result)))
  ...
  (cond ((tree-is? line 'algo-if)
         (let ((cond-arg (arg0 line))
           ...
           (if (== body-arg "")
               ...
               (cons (algo-header '(render-if) cond-arg '(render-then) "")
                     (let ((result (ext-apply-on-paragraphs "numbered-line" cond-arg)))
                       (if (tm-func? result 'document)
                           (tm-children result)
                           (list result))))
               (cons (algo-header '(render-if) cond-arg '(render-then) "")
                     (let ((result (ext-apply-on-paragraphs "numbered-line" body-arg)))
                       (if (tm-func? result 'document)
                           (tm-children result)
                           (list result)))))))
        ((tree-is? line 'algo-else-if)
        ...
        ((tree-is? line 'algo-else)
         (cons `(numbered-line (concat (render-else) (no-page-break)))
               (let ((result (ext-apply-on-paragraphs "numbered-line" (arg0 line))))
                 (if (tm-func? result 'document)
                     (tm-children result)
                     (list result)))))
        ((tree-is? line 'algo-while)
        ...
        ((tree-is? line 'algo-procedure)
         (append (list `(numbered-line (concat (render-procedure) " " (with "font-shape" "small-caps" ,(arg0 line)) "(" ,(arg1 line) ")" (no-page-break))))
                 (let ((result (ext-apply-on-paragraphs "numbered-line" (arg2 line))))
                   (if (tm-func? result 'document)
                       (tm-children result)
                       (list result)))
