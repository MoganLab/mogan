...
(define (wrap-algo-macro line lang)
  ...
  (cond ((tree-is? line 'algo-if)
         (let ((cond-arg (arg0 line))
               (body-arg (arg1 line)))
           (if (== body-arg "")
               ;; only one argument: treat it as body
  ...
  (let ((l (tm-children line)))
    (if (< (length l) 2)
        ...
        (let ((cond-arg (car l))
              (body-arg (cadr l))
              (rest (cddr l)))
          (append (list `(numbered-line (concat (render-if) " " ,cond-arg " " (render-then) (no-page-break))))
                  (wrap-algo-body body-arg lang)
                  (let loop ((r rest))
                    (cond ((null? r)
                          ...
                          ((null? (cdr r))
                           (cons `(numbered-line (concat (render-else) (no-page-break)))
                                 (wrap-algo-body (car r) lang)))
                          (else
                           (let ((ei-cond (car r))
                                 (ei-body (cadr r))
                                 (ei-rest (cddr r)))
                             (append (list `(numbered-line (concat (render-else) " " (render-if) " " ,ei-cond " " (render-then) (no-page-break))))
                                     (wrap-algo-body ei-body lang)
                                     (loop ei-rest)))))))))))
