...
(define (wrap-algo-macro line lang)
  (cond ((tree-is? line 'algo-if)
         (cons `(numbered-line (concat (render-if) " " ,(tm-ref line 0) " " (render-then) (no-page-break)))
               (wrap-algo-body (tm-ref line 1) lang)))
        ((tree-is? line 'algo-else-if)
         (cons `(numbered-line (concat (render-else) " " (render-if) " " ,(tm-ref line 0) " " (render-then) (no-page-break)))
               (wrap-algo-body (tm-ref line 1) lang)))
        ((tree-is? line 'algo-else)
         (cons `(numbered-line (concat (render-else) (no-page-break)))
               (wrap-algo-body (tm-ref line 0) lang)))
        ((tree-is? line 'algo-while)
         (append (list `(numbered-line (concat (render-while) " " ,(tm-ref line 0) " " (render-do) (no-page-break))))
                 (wrap-algo-body (tm-ref line 1) lang)
                 (list `(numbered-line (concat (render-end-while) (right-flush))))))
        ((tree-is? line 'algo-for)
         (append (list `(numbered-line (concat (render-for) " " ,(tm-ref line 0) " " (render-do) (no-page-break))))
                 (wrap-algo-body (tm-ref line 1) lang)
                 (list `(numbered-line (concat (render-end-for) (right-flush))))))
        ((tree-is? line 'algo-for-all)
         (append (list `(numbered-line (concat (render-for-all) " " ,(tm-ref line 0) " " (render-do) (no-page-break))))
                 (wrap-algo-body (tm-ref line 1) lang)
                 (list `(numbered-line (concat (render-end-for) (right-flush))))))
        ((tree-is? line 'algo-for-each)
         (append (list `(numbered-line (concat (render-for-each) " " ,(tm-ref line 0) " " (render-do) (no-page-break))))
                 (wrap-algo-body (tm-ref line 1) lang)
                 (list `(numbered-line (concat (render-end-for) (right-flush))))))
        ((tree-is? line 'algo-repeat)
         (append (list `(numbered-line (concat (render-repeat) (no-page-break))))
                 (wrap-algo-body (tm-ref line 1) lang)
                 (list `(numbered-line (concat (render-until) " " ,(tm-ref line 0) (right-flush))))))
        ((tree-is? line 'algo-loop)
         (append (list `(numbered-line (concat (render-loop) (no-page-break))))
                 (wrap-algo-body (tm-ref line 1) lang)
                 (list `(numbered-line (concat (render-end-loop) (right-flush))))))
        ((tree-is? line 'algo-procedure)
         (append (list `(numbered-line (concat (render-procedure) " " (with "font-shape" "small-caps" ,(tm-ref line 0)) "(" ,(tm-ref line 1) ")" (no-page-break))))
                 (wrap-algo-body (tm-ref line 2) lang)
                 (list `(numbered-line (concat (render-end-procedure) (right-flush))))))
        ((tree-is? line 'algo-function)
         (append (list `(numbered-line (concat (render-function) " " (with "font-shape" "small-caps" ,(tm-ref line 0)) "(" ,(tm-ref line 1) ")" (no-page-break))))
                 (wrap-algo-body (tm-ref line 2) lang)
        ...
        ((tree-is? line 'algo-outputs)
         (cons `(numbered-line (concat (render-outputs) (no-page-break)))
               (wrap-algo-body (tm-ref line 0) lang)))
        ((tree-is? line 'algo-if-else-if)
         (wrap-algo-if-else-if line lang))
        (else (list line))))
...
(define (wrap-algo-if-else-if line lang)
  (let ((l (tm-children line)))
    (if (< (length l) 2)
        (list)
        (append (list `(numbered-line (concat (render-if) " " ,(car l) " " (render-then) (no-page-break))))
                (wrap-algo-body (cadr l) lang)
                (let loop ((rest (cddr l)))
                  (cond ((null? rest)
                         (list `(numbered-line (concat (render-end-if) (right-flush)))))
                        ((null? (cdr rest))
                         (cons `(numbered-line (concat (render-else) (no-page-break)))
                               (wrap-algo-body (car rest) lang)))
                        (else
                         (append (list `(numbered-line (concat (render-else) " " (render-if) " " ,(car rest) " " (render-then) (no-page-break))))
                                 (wrap-algo-body (cadr rest) lang)
                                 (loop (cddr rest))))))))))
