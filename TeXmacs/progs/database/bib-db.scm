...
(define (db-bib-keep-case s f?)
  (if (or (== s (locase-all s)) (and f? (== s (upcase-first (locase-all s)))))
    s
    `(keepcase ,s)
  ) ;if
) ;define
...
(define (db-bib-spaces? t)
  (cond ((string? t) (list-and (map (cut == <> "") (string-decompose t " "))))
        ((tm-func? t 'concat) (list-and (map db-bib-spaces? (tm-children t))))
        ((with-like? t) (db-bib-spaces? (cAr t)))
        (else #f)
  ) ;cond
) ;define
...
(define (db-bib-protect t f?)
  (cond ((tm-atomic? t)
         (let* ((l (string-decompose t " ")) (r (db-bib-protect-strings l f?)))
           (apply tmconcat (list-intersperse r " "))
         ) ;let*
        ) ;
        ((tm-func? t 'concat)
         (with l (db-bib-protect-list (tm-children t) f?) (apply tmconcat l))
        ) ;
...
(define (db-bib-name t)
  (let* ((c (if (tm-func? t 'concat) (tm-children t) (list t)))
         (fi (list-find-index c
               (lambda (x)
                 (or (tm-func? x 'name 1) (tm-func? x 'name-von 1) (tm-func? x 'name-jr 1))
               ) ;lambda
             ) ;list-find-index
         ) ;fi
...
(define (db-bib-sub-sub type var val)
  (cond ((and (== var "title")
        ...
        ((or (== var "type") (== var "mtype")) (db-bib-protect val #f))
        ((or (== var "author") (== var "editor")) (db-bib-names val))
        ((== var "pages") (db-bib-pages val))
        (else val)
  ) ;cond
) ;define
...
(define (db-bib-sub t type)
  (cond ((tm-func? t 'db-field 2)
         (let* ((var (tm-ref t 0)) (val (tm-ref t 1)))
...
(tm-define (db->bib t)
  ...
  (cond ((and (db-entry-any? t)
         ...
         (let* ((type (tm-ref t 1))
                (name (tm-ref t 2))
                (l (map (cut db-bib-sub <> type) (tm-children (tm-ref t :last))))
               ) ;
...
(tm-define (as-bib)
  (with t
    ...
    (when (tm-func? t 'document)
      (with r
        `(document ,@(map db->bib (tm-children t)))
        (tree-assign! t r)
...
(define (bib-db-unmacro t)
  (cond ((tm-atomic? t) (tm->string t))
        ((tm-func? t 'keepcase 1) (bib-db-unmacro (tm-ref t 0)))
        ((tm-func? t 'bib-names)
         (let* ((l (map bib-db-unmacro (tm-children t))) (r (list-intersperse l '(name-sep))))
           (apply tmconcat r)
         ) ;let*
        ) ;
        ((tm-func? t 'bib-name 4)
         (with (f v l j)
           (tm-children (tm->stree t))
           (with r
             (append (if (== f "") (list) (list f))
               (if (== v "") (list) (list " " `(name-von ,v)))
               (if (== l "") (list) (list " " `(name ,l)))
               (if (== j "") (list) (list " " `(name-jr ,j)))
             ) ;append
             (while (and (nnull? r) (== (car r) " ")) (set! r (cdr r)))
             (apply tmconcat (map bib-db-unmacro r))
           ) ;with
         ) ;with
        ) ;
        ((tm-func? t 'bib-pages)
         (let* ((l (map bib-db-unmacro (tm-children t))) (r (list-intersperse l "\x15;")))
           (apply tmconcat r)
         ) ;let*
        ) ;
        ((tm-func? t 'concat)
         (with l (map bib-db-unmacro (tm-children t)) (apply tmconcat l))
        ) ;
        (else (cons (tm-label t) (map bib-db-unmacro (tm-children t))))
  ) ;cond
) ;define

(define (bib-db-locase t first?)
  (cond ((tm-atomic? t)
         (with r (locase-all (tm->string t)) (if first? (upcase-first r) r))
        ) ;
        ((or (tm-func? t 'verbatim) (tm-func? t 'slink)) t)
        ((and (tm-func? t 'with 3)
           (tm-equal? (tm-ref t 0) "font-family")
           (tm-equal? (tm-ref t 1) "tt")
         ) ;and
         t
        ) ;
        ((and (tm-func? t 'with 3) (tm-equal? (tm-ref t 0) "math-font")) t)
...
(define (bib-db-sub-sub type var val)
  (cond ((and (== var "title")
        ...
        ((or (== var "type") (== var "mtype")) (bib-db-locase val #f))
        (else val)
  ) ;cond
) ;define
...
(define (db-get-origin)
  (if db-bib-origin (list `(fb-field ,"origin" ,db-bib-origin)) (list))
) ;define

(tm-define (bib->db t)
  (cond ((and (tm-func? t 'bib-entry 3) (tm-func? (tm-ref t 2) 'document))
         (let* ((id (with-database (bib-database) (db-create-id)))
                ...
                (l (map (cut bib-db-sub <> type*) (tm-children (tm-ref t 2))))
               ) ;
...
(tm-define (db-save-pre t) (former (bib->db t)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Load and save bibliographies
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(tm-define bib-types-list (smart-ref db-kind-table "bib"))

(tm-define (bib-load) (db-load-types bib-types-list))

(tm-define (bib-save t) (db-save-types t bib-types-list))
...
(tm-define (tmbib-snippet->texmacs s)
  (with t (bibtex->texmacs (parse-bibtex-snippet s)) (bib->db* t))
) ;tm-define

(define (tmbib-import s att?)
  (let* ((t (bibtex->texmacs (parse-bibtex-document s)))
         (doc (bib->db* t))
         (tm (if (tmfile-extract doc 'TeXmacs) (list) (list `(TeXmacs ,(texmacs-version))))
         ) ;tm
...
(tm-define (tmbib-document->texmacs s) (tmbib-import s #t))

(tm-define (tmbib-document->texmacs* s) (tmbib-import s #f))
...
(tm-define (texmacs->tmbib-snippet t)
  (with u (db->bib* t) (serialize-bibtex (texmacs->bibtex u)))
) ;tm-define
