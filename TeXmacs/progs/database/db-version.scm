...
(tm-define (db-same-entries? e1 e2)
  (let* ((ok?
           (lambda (f) (and (pair? f) (nin? (car f) (db-meta-attributes))))
         ) ;ok?
  ...
  (with old-l
    ...
    (if (db-same-entries? new-l old-l)
      ...
      (let* ((new-h (cons id (or (assoc-ref new-l "newer") (list))))
        ...
        (with new-id
          (if
            (and (nnull? opt-new-id) (not (db-entry-exists? (car opt-new-id))))
            (begin
...
(define (db-find-entry l)
  (let* ((ids
           (db-search (list (cons "name" (assoc-ref l "name"))))
         ) ;ids
...
(tm-define (db-import-entry id l)
  (let* ((name (assoc-ref-first l "name"))
    ...
    (cond
     ((nnull? (db-get-entry id))
      ;; new entries must have new identifiers
      (when
        (and db-duplicate-warning? (not (db-same-entries? l (db-get-entry id))))
