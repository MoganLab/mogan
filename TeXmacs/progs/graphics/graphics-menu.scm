...
(menu-bind graphics-mode-menu
 ...
 ("Arc" (graphics-set-mode '(edit std-arc-counterclockwise)))
...
(menu-bind graphics-color-menu
  ...
  ---
  (if (allow-pattern-colors?) (pick-background "1gu" (graphics-set-color answer)))
  (if (not (allow-pattern-colors?)) (pick-color (graphics-set-color answer)))
  ---
...
(menu-bind grid-color-axes-menu
 ...
 ---
 ("Palette"
   (interactive-color (lambda (c) (graphics-set-grid-color 'axes c)) '())
 ) ;
 ("Other" (interactive (lambda (x) (graphics-set-grid-color 'axes x)) "Color"))
) ;menu-bind
...
(menu-bind grid-color-units-menu
 ...
 ---
 ("Palette"
   (interactive-color (lambda (c) (graphics-set-grid-color 'units c)) '())
 ) ;
 ("Other" (interactive (lambda (x) (graphics-set-grid-color 'units x)) "Color"))
) ;menu-bind
...
(menu-bind grid-color-subunits-menu
 ...
 ---
 ("Palette"
   (interactive-color (lambda (c) (graphics-set-grid-color 'subunits c)) '())
 ) ;
 ("Other"
   (interactive (lambda (x) (graphics-set-grid-color 'subunits x)) "Color")
 ) ;
) ;menu-bind
...
(menu-bind graphics-fill-color-menu
  ...
  (if (not (allow-pattern-colors?)) (pick-color (graphics-set-fill-color answer)))
  ---
...
(menu-bind graphics-insert-menu
  ...
  (assuming (inside? 'screens) (-> "Slides" (link graphics-screens-menu)))
...
(menu-bind graphics-focus-menu
  ...
  (assuming (graphics-get-anim-type) (-> "Status" (link graphics-anim-type-menu)))
  ---
...
(tm-menu (graphics-group-icons)
 ((check (balloon (icon "tm_edit_props.xpm") "Edit object properties")
    "v"
    (== (graphics-mode) '(group-edit edit-props))
  ) ;check
 ...
 (assuming (not (tree-innermost user-anim-context?))
  ((check (balloon (icon "tm_animate.xpm") "Animate object")
     "v"
     (== (graphics-mode) '(group-edit animate))
   ) ;check
  ...
  (mini #t
    ...
    (if (!= (graphics-get-grid-type #t) 'empty)
      (if (graphics-get-snap "grid point")
       ((balloon (icon "tm_snap_grid.xpm") "Snap to grid")
        (graphics-reset-snap "grid point")
       ) ;
      ) ;if
      (if (graphics-get-snap "grid curve point")
       ((balloon (icon "tm_snap_grid_curve.xpm") "Snap to grid curve")
    ...
    (if (graphics-get-snap "curve point")
     ((balloon (icon "tm_snap_curve.xpm") "Snap to curves")
    ...
    (if (graphics-get-snap "text border point")
     ((balloon (icon "tm_snap_text_border.xpm") "Snap to text corners")
      (graphics-reset-snap "text border point")
     ) ;
    ) ;if
    (if (graphics-get-snap "text border")
     ((balloon (icon "tm_snap_text_deco.xpm") "Snap to text borders")
...
(define (gr-mode->string s)
  (cond ((== s '(edit point)) "point")
        ...
        ((== s '(edit std-arc)) "std-arc")
        ((== s '(edit std-arc-counterclockwise)) "std-arc")
        ((== s '(edit sector)) "sector")
...
(tm-menu (graphics-focus-icons)
  ...
  (assuming (graphics-get-anim-type)
    ...
    (mini #t
      (group "Status:")
      (=> (eval (graphics-get-anim-type)) (link graphics-anim-type-menu))
    ) ;mini
  ) ;assuming
  /
...
(tm-menu (focus-hidden-menu t)
  ...
  (assuming (hidden-child? t 2) (dynamic (string-input-menu t 2)))
) ;tm-menu

(tm-menu (focus-hidden-icons t)
  (:require (tree-in? t '(draw-over draw-under)))
  (glue #f #f 10 0)
  ((balloon (icon "tm_enter_image.xpm") "Enter graphics mode") (graphics-enter))
  (assuming (hidden-child? t 2) (dynamic (string-input-icon t 2)))
) ;tm-menu
...
(menu-bind doc-at-fill-color-menu
 ...
 (if (not (allow-pattern-colors?)) (pick-color (object-set-fill-color answer)))
 ---
...
(tm-menu (focus-hidden-icons t)
  ...
  (mini #t
    (group "Alignment:")
    (with s
      (object-get-property "text-at-halign")
      (=> (eval s) (link text-at-halign-menu))
    ) ;with
    (with s
      (object-get-property "text-at-valign")
...
(tm-menu (focus-hidden-icons t)
  ...
  (mini #t
    (group "Fill color:")
    (with col
      (object-get-property "fill-color")
      (assuming (in? col (list "" "none" "default" "mixed"))
        (=> (eval (if (== col "mixed") col "none")) (link doc-at-fill-color-menu))
      ) ;assuming
      (assuming (nin? col (list "" "none" "default" "mixed"))
        (=> (color col #f #f 25 17) (link doc-at-fill-color-menu))
      ) ;assuming
    ) ;with
  ) ;mini
  /
  (mini #t
    ...
    (with s
      (object-get-property "doc-at-valign")
