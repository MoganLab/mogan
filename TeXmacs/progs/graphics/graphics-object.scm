...
(define (is-point? p)
  (and (pair? p) (in? (car p) '(point tuple)) (pair? (cdr p)) (npair? (cadr p)))
) ;define

(define (graphics-closest-point-pos-bis p l)
  (if (null? l)
    '()
    (let ((n1 (if (is-point? (car l)) (graphics-norm p (car l)) infinity)))
      (if (null? (cdr l))
        l
        (let* ((p2 (graphics-closest-point-pos-bis p (cdr l)))
               (n2 (if (is-point? (car p2)) (graphics-norm p (car p2)) infinity))
              ) ;
          (if (<= n1 n2) l p2)
        ) ;let*
      ) ;if
...
(tm-define (default-color-go-points)
  (if (has-style-package? "dark") "#3A86FF" "#4040ff")
) ;tm-define
...
(tm-define graphical-attrs
  (with t
    (make-ahash-table)
    (for (var (graphics-all-attributes)) (ahash-set! t var "default"))
    t
  ) ;with
) ;tm-define
...
(tm-define (graphical-object fetch)
  (with o
    (tree->stree (get-graphical-object))
    (if (and fetch (pair? o)) (graphical-fetch-props o))
    (if (pair? o) (if (== (cdr o) '()) o (cAr o)) '(concat))
  ) ;with
) ;tm-define
...
(define (dv var val)
  (if (== val "default") (get-default-val var) val)
) ;define
...
(tm-define (get-graphical-prop mode prop)
  (with res
    (cond ((== mode 'basic)
          ...
          ((== mode 'active) (ahash-ref graphical-attrs prop))
          ((list? mode)
           (if (== prop "magnify")
          ...
          ((== mode 'new) (graphics-get-property (string-append "gr-" prop)))
          (else
            ;; (== mode 'default)
...
(define (create-graphical-embedding-box o ha0 va0 halign valign w hm pp mag)
  (define (create-text-at-handle o)
    (cond ((func? o 'with) (create-text-at-handle (cAr o)))
          ((graphical-text-at-context? o) `((with ,"point-style"
                                              ,"disk"
                                              ,(cAr o))))
          (else '())
    ) ;cond
...
(define (create-graphical-contour* o edge no)
  ...
  (cond ((== (car o) 'point) (cons o '()))
        ...
        ((in? (car o) '(anim-edit)) (create-graphical-contour* (caddr o) 0 #f))
...
(define (compress* l)
  (if (<= (length l) 50) l (compress* (compress l)))
) ;define
...
(define (create-graphical-contours l ptr pts)
  ...
  (define (asc col fcol op)
    (if (and on-aobj (not aobj-selected)) (set! fcol #f))
  ...
  (for (o l)
    (if (tree? o)
      (with path (reverse (tree-ip o)) (if (== path ptr) (set! aobj-selected #t)))
    ) ;if
  ) ;for
      ...
      (let* ((props #f) (t #f) (path0 #f))
        ...
        (if (tree? o)
          (with path
            (reverse (tree-ip o))
            (set! props
              (create-graphical-props (if (== pts 'points) 'default path)
                (if (== pts 'object) #f "square")
              ) ;create-graphical-props
            ) ;set!
            (if (or (== path ptr) (and (list? path) (list? ptr) (list-starts? path ptr)))
              (begin
                (set! on-aobj #t)
...
(tm-define (create-graphical-object o mode pts no)
  ...
  (if (graphical-text-arg-context? o) (set! o #f))
  (if (pair? no) (begin (set! edge (car no)) (set! no (cadr no))))
  (if (and o (not (null? o)))
    (let* ((op (add-selections-colors (create-graphical-contour o edge no)
                 (default-color-go-points)
                 #f
...
(tm-define (graphics-decorations-update . parms)
  ...
  (if (== (length parms) 4)
    (create-graphical-object (car parms) (cadr parms) (caddr parms) (cadddr parms))
    (if (graphics-group-mode? (graphics-mode))
      (with pts
        ...
        (if (and (null? (sketch-get)) (pair? current-obj) (> (length current-obj) 1))
      ...
      (let* ((mode (if (null? parms) (if sticky-point 'active current-path) (car parms)))
        ...
        (create-graphical-object current-obj
          mode
...
(tm-define (transform-graphical-object opn)
  (with o
    (tree->stree (get-graphical-object))
    (if (pair? o) (begin (set! o (opn o)) (graphical-object! o)))
  ) ;with
) ;tm-define
...
(define (get-root x)
  (if (and (tree? x) (tree->path x)) (radical->enhanced-tree x) x)
) ;define

(tm-define (sketch-get) the-sketch)

(tm-define (sketch-set! l) (set! the-sketch (map get-root l)))

(tm-define (sketch-in? t) (set! t (get-root t)) (seek-eq? t the-sketch))

(tm-define (sketch-reset) (set! the-sketch '()))

(tm-define (sketch-toggle t)
  (set! t (get-root t))
  (set! the-sketch
    (if (sketch-in? t) (remove-eq? t the-sketch) (rcons the-sketch t))
  ) ;set!
) ;tm-define

(tm-define (sketch-transform opn)
  (set! the-sketch (map opn the-sketch))
  (set! current-obj (if (graphics-group-mode? (graphics-mode)) '(nothing) #f))
  (set! current-path #f)
...
(tm-define (sketch-checkout)
  (if sticky-point (graphics-error "(sketch-checkout)"))
  (set! layer-of-last-removed-object (if (== (length (sketch-get)) 1) #f '()))
...
(tm-define (sketch-commit)
  ...
  (if (not sticky-point) (graphics-error "(sketch-commit)"))
  (with sketch0
  ...
  (if remove-undo-mark? (remove-undo-mark))
  (graphics-decorations-update)
) ;tm-define
