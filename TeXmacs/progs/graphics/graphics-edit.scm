...
(tm-define (graphics-graphical? t)
  (and (tree? t)
    (or (tree-in? t gr-tags-all)
      (and (tree-is? t 'with) (graphics-graphical? (tree-ref t :last)))
    ) ;or
  ) ;and
) ;tm-define
...
(tm-define (graphics-graphical-index t)
  (or (and (tree? t)
        (tree-compound? t)
        (list-find-index (tree-children t) graphics-graphical?)
      ) ;and
    0
  ) ;or
) ;tm-define
...
(tm-define (graphics-zprevious p)
  (let* ((p0 (cDr p))
    ...
    (foreach-number (i (- ofs 1) >= i0)
      (with t
        (path->tree (rcons p0 i))
        (if (and (not res) (box-intersects t (path->tree p))) (set! res i))
      ) ;with
    ) ;foreach-number
    res
  ) ;let*
...
(tm-define (graphics-znext p)
  (let* ((p0 (cDr p)) (t0 (path->tree p0)) (ofs (cAr p)) (res #f))
...
(tm-define (graphics-enter-mode old-mode new-mode)
  ...
  (if (and (graphics-group-mode? old-mode) (not (graphics-group-mode? new-mode)))
    (graphics-reset-state)
  ) ;if
  (if (and (not (graphics-group-mode? old-mode)) (graphics-group-mode? new-mode))
    (begin
...
(tm-define (graphics-busy?) (:state graphics-state) sticky-point)
...
(tm-define (set-keyboard-modifiers mods) (set! the-keyboard-modifiers mods))

(tm-define (get-keyboard-modifiers) the-keyboard-modifiers)
...
(tm-define (graphics-release-left x y)
  ...
  (if (inside-graphical-text?)
    (with-innermost t
      graphical-text-context?
      (let* ((ps (select-first (s2f x) (s2f y))) (p (and ps (car ps))))
        (if (and p (list-starts? p (tree->path t))) (go-to p) (tree-go-to t :start))
      ) ;let*
    ) ;with-innermost
...
(tm-define (graphics-start-drag-left x y t p)
  ...
  (when (not (inside-graphical-text?))
    (edit_start-drag (car (graphics-mode)) x y t p)
  ) ;when
) ;tm-define

(tm-define (graphics-dragging-left x y t p)
  ;; (display* "Graphics] Dragging-left " x ", " y ", " t ", " p "\n")
  (when (not (inside-graphical-text?))
    (edit_drag (car (graphics-mode)) x y t p)
  ) ;when
) ;tm-define

(tm-define (graphics-end-drag-left x y t p)
  ;; (display* "Graphics] End-drag-left " x ", " y ", " t ", " p "\n")
  (when (not (inside-graphical-text?))
    (edit_end-drag (car (graphics-mode)) x y t p)
  ) ;when
) ;tm-define
