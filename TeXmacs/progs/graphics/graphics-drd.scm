...
(texmacs-module (graphics graphics-drd)
  (:use (utils library cursor) (utils library tree) (utils edit variants))
) ;texmacs-module
...
(define-group variant-tag (graphical-over-under-tag))
...
(define-group graphical-curve-tag
  (graphical-open-curve-tag)
  (graphical-closed-curve-tag)
) ;define-group

(define-group graphical-short-text-tag text-at math-at)

(define-group graphical-long-text-tag document-at)

(define-group graphical-text-tag
  (graphical-short-text-tag)
  (graphical-long-text-tag)
) ;define-group

(define-group graphical-penscript-tag penscript)

(define-group graphical-calligraphy-tag calligraphy)

(define-group graphical-pen-tag
  (graphical-penscript-tag)
  (graphical-calligraphy-tag)
) ;define-group

(define-group graphical-contains-curve-tag (graphical-curve-tag))

(define-group graphical-contains-text-tag (graphical-text-tag))

(define-group graphical-non-group-tag
  (graphical-curve-tag)
  (graphical-atomic-tag)
  (graphical-text-tag)
) ;define-group

(define-group graphical-group-tag gr-group)

(define-group graphical-tag (graphical-non-group-tag) (graphical-group-tag))

(define-group graphical-over-under-tag draw-over draw-under)

(tm-define (graphical-context? t) (tm-in? t (graphical-tag-list)))

(tm-define (graphical-text-context? t) (tm-in? t (graphical-text-tag-list)))
...
(tm-define (inside-graphical-text?)
  (and-with t
    ...
    (and-with u
      (tree-ref t :down)
      (and-with i (tree-index u) (tree-accessible-child? t i))
    ) ;and-with
  ) ;and-with
) ;tm-define
...
(tm-define gr-tags-noncurves
  (append (graphical-atomic-tag-list)
    (graphical-text-tag-list)
    (graphical-group-tag-list)
  ) ;append
) ;tm-define

(tm-define (graphical-user-tag? l) (in? l gr-tags-user))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; List of graphical attributes and their properties
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(tm-define (gr-prefixed? attr) (string-starts? attr "gr-"))

(tm-define (gr-prefix attr) (string-append "gr-" attr))

(tm-define (gr-unprefix attr) (substring attr 3 (string-length attr)))
...
(tm-define (graphics-attributes tag) (graphics-common-attributes))
...
(tm-define (graphics-attributes tag)
  ...
  (append (graphics-common-attributes)
    '("text-at-halign" "text-at-valign" "text-at-repulse" "text-at-snapping")
  ) ;append
) ;tm-define
...
(tm-define (graphics-attributes tag)
  ...
  (append (graphics-common-attributes) '("pen-enhance" "pen-style" "line-width"))
) ;tm-define
...
(tm-define (graphics-attribute? tag attr) (in? attr (graphics-attributes tag)))
...
(tm-define (graphics-test-anim-type? val) (== (graphics-get-anim-type) val))
...
(tm-define (graphics-complete obj) (list obj #f))
