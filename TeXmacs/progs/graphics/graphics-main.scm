...
(texmacs-module (graphics graphics-main) (:use (graphics graphics-utils)))
...
(tm-define (graphics-enter-into t)
  (set! t (tree-ref t 1))
  (while (tree-is? t 'with) (set! t (tm-ref t :last)))
  (cond ((tm-equal? t '(graphics)) (tree-go-to t :end))
        ((tree-is? t 'graphics) (tree-go-to t :last :end))
  ) ;cond
) ;tm-define
...
(tm-define (graphics-exit-right)
  ...
  (cond ((inside-graphical-over-under?)
         (with-innermost t graphical-over-under-context? (tree-go-to t :end))
        ) ;
        ((inside? 'graphics)
         (with-innermost t
           'graphics
           (while (and (tree-up t) (tree-func? (tree-up t) 'with)) (set! t (tree-up t)))
           (tree-go-to t :end)
         ) ;with-innermost
        ) ;
...
(tm-define (graphics-set-overlap w)
  ...
  (when (inside-graphical-over-under?)
    (with-innermost t graphical-over-under-context? (tree-set t 2 w))
  ) ;when
) ;tm-define

(tm-define (graphics-geometry)
  (with geo
    (tree->stree (get-env-tree "gr-geometry"))
...
(define (geo-valign-has-value? val)
  (let* ((geo (graphics-geometry)) (align (car (cddddr geo))))
    (== val align)
  ) ;let*
) ;define
...
(tm-define (graphics-cartesian-frame)
  (with frame
    (tree->stree (get-env-tree "gr-frame"))
...
(define (graphics-unit-has-value? val)
  (let* ((fr (graphics-cartesian-frame)) (unit (caddr fr)))
    (== val unit)
  ) ;let*
) ;define
...
(tm-define (graphics-set-unit u)
  ...
  (with frame
    (graphics-cartesian-frame)
    (with new-frame
      `(tuple ,"scale" ,u ,(cAr frame))
...
(define (graphics-origin-has-value? x y)
  (let* ((fr (graphics-cartesian-frame)) (orig (cAr fr)))
    (if (pair? x) (set! x (length-add (cadr x) (caddr x))))
    (if (pair? y) (set! y (length-add (cadr y) (caddr y))))
...
(tm-define (length-extract-unit len)
  ...
  (define (traverse l)
    (if (pair? l)
      (if (char-alphabetic? (car l)) (traverse (cdr l)) (set-cdr! l '()))
    ) ;if
  ) ;define
  (traverse l)
  (set! l (reverse l))
...
(tm-define (graphics-toggle-auto-crop)
  ...
  (with new
    (if (graphics-auto-crop?) "false" "true")
...
(tm-define (graphics-set-zoom z) (graphics-zoom (/ z (graphics-get-zoom))))
...
(tm-define (graphics-change-extents dw dh)
  (when (not (graphics-auto-crop?))
    (let* ((geo (graphics-geometry))
      ...
      (if (> (length-decode w2) 0) (set! w w2))
...
(tm-define (graphics-set-notebook-grid)
  ...
  (delayed (:idle 1)
    ...
    (delayed (:idle 1) (graphics-set-grid-color 'axes "#e0e0ff"))
  ) ;delayed
) ;tm-define
...
(define (graphics-fetch-grid-vars type visual?)
  ...
  (set! graphics-current-step
    (if (or visual? (== graphics-current-type "logarithmic")) "1" "0.1")
  ) ;set!
  (set! graphics-current-astep
    (f2s (if visual? default-polar-astep (* default-polar-astep default-polar-nsubd))
    ) ;f2s
  ) ;set!
  (set! graphics-current-base "10")
  (with grid
    (tree->stree (get-env-tree (if visual? "gr-grid" "gr-edit-grid")))
    (cond ((match? grid '(tuple "empty")) (set! graphics-current-type "empty"))
          ((match? grid '(tuple "cartesian")) (set! graphics-current-type "cartesian"))
          ...
          ((match? grid '(tuple "polar")) (set! graphics-current-type "polar"))
...
(define (graphics-set-grid visual?)
  (let* ((type (string->symbol graphics-current-type))
    ...
    (cond ((== type 'empty) (set! the-grid '(tuple "empty")))
          ((== type 'cartesian) (set! the-grid `(tuple ,"cartesian"
                                                  ,center
                                                  ,step)))
          ((== type 'polar) (set! the-grid `(tuple ,"polar"
                                              ,center
                                              ,step
                                              ,astep)))
          ((== type 'logarithmic)
    ...
    (if the-grid
      (with grid-old
        (tree->stree (get-env-tree prop-old))
        (if (and (== (get-actual-grid-type visual?) "empty")
          ...
          (begin
            ...
            (if (!= type 'empty) (graphics-set-property prop-old the-grid))
          ) ;begin
        ) ;if
      ) ;with
    ) ;if
...
(tm-define (graphics-set-visual-grid type)
  ...
  (with new-polar?
    #f
    (if (and (== type 'polar) (!= type (string->symbol graphics-current-type)))
      (let* ((aspect (graphics-grid-aspect #t)) (nsubds (aspect-ref aspect 3)))
...
(define (update-edit-grid cmd)
  (if egrid-as-vgrid? (graphics-set-edit-grid cmd))
) ;define
...
(tm-define (graphics-interactive-set-grid-center visual?)
  (:interactive #t)
  (interactive (lambda (x y) (graphics-set-grid-center x y visual?))
...
(tm-define (graphics-interactive-set-grid-step visual?)
  (:interactive #t)
  (interactive (lambda (x) (graphics-set-grid-step x visual?)) "Unit length")
) ;tm-define
...
(tm-define (graphics-interactive-set-grid-base visual?)
  (:interactive #t)
  (interactive (lambda (x) (graphics-set-grid-base x visual?)) "Logarithmic base")
) ;tm-define
...
(tm-define (graphics-set-grid-aspect-properties c0 c1 s2 c2)
  ...
  (with aspect
    `(tuple (tuple ,"axes" ,c0) (tuple ,"1" ,c1) (tuple ,s2 ,c2))
...
(define (cmp-aspect-items x y)
  (if (== (cadr x) "axes")
    ...
    (if (== (cadr y) "axes")
      #f
      (let* ((xval (s2f (cadr x))) (yval (s2f (cadr y))))
        (< xval yval)
      ) ;let*
    ) ;if
  ) ;if
...
(define (graphics-grid-aspect-props)
  ...
  (with aspect
    ...
    (get-upwards-property (cDr (cursor-path)) "gr-grid-aspect-props")
    ...
    (if (match? aspect '(tuple (tuple :%2) (tuple :%2) :*))
      (set! res aspect)
      (begin
        (set! aspect (graphics-path-property (graphics-graphics-path) "gr-grid-aspect"))
        (if (match? aspect '(tuple (tuple :%2) (tuple :%2) :*))
          (set! res aspect)
...
(define (nsubd-has-value? type nsubd visual?)
  (with aspect
    (graphics-grid-aspect visual?)
    (with ref
      (aspect-ref aspect 3)
      (if ref
        (if (number? nsubd)
          (== (number->string nsubd) (cadr ref))
          (let* ((aspect (get-default-val (if visual? "gr-grid-aspect" "gr-edit-grid-aspect")))
...
(tm-define (graphics-set-grid-aspect type nsubd visual?)
  ...
  (if visual?
            ...
            ((== type 'detailed)
             (if nsubd
               (set-car! (cdr (list-ref aspect 3))
                 (or (and (string? nsubd) nsubd) (number->string nsubd))
               ) ;set-car!
               (set-car! (cdr (list-ref aspect 3))
                 (cadr (list-ref (get-default-val "gr-grid-aspect") 3))
               ) ;set-car!
             ) ;if
             (graphics-set-property "gr-grid-aspect" aspect)
             (graphics-set-property "gr-grid-aspect-props" aspect)
    ...
    (with aspect
      ...
      (if (not nsubd)
        (set-car! (cdr (list-ref aspect 3))
          (cadr (list-ref (get-default-val "gr-edit-grid-aspect") 3))
        ) ;set-car!
      ) ;if
      (graphics-set-property "gr-edit-grid-aspect" aspect)
      (if (!= type 'update) (grid-as-visual-grid! #f))
    ) ;with
  ) ;if
) ;tm-define
...
(tm-define (graphics-interactive-set-grid-nsubds visual?)
  (:interactive #t)
  (interactive (lambda (x) (graphics-set-grid-aspect 'detailed (string->number x) visual?))
...
(define (grid-color-has-value? where color)
  (let* ((i (grid-aspect-ofs (cadr where)))
    ...
    (if (== color "default")
      (let* ((aspect (get-default-val "gr-grid-aspect")) (ref2 (aspect-ref aspect i)))
        (and ref2
          (or ref ref-props)
          (== (caddr ref2) (if ref (caddr ref) (caddr ref-props)))
        ) ;and
      ) ;let*
      (if ref (== color (caddr ref)) (== color (caddr ref-props)))
    ) ;if
  ) ;let*
) ;define
...
(define (grid-aspect-show-subunits?)
  (with aspect
    (tree->stree (get-env-tree "gr-grid-aspect"))
...
(define (graphics-toggle-grid* visual?)
  (let* ((prop (if visual? "gr-grid" "gr-edit-grid"))
    ...
    (if (!= gr-old nothing)
      (if (or (== gr nothing) (== (cadr gr) "empty"))
...
(define (graphics-mode-has-value? mode)
  (if (and (pair? mode) (eq? (car mode) 'quote)) (set! mode (cadr mode)))
  (== mode (graphics-mode))
) ;define
...
(tm-define (graphics-group-mode? mode) (func? mode 'group-edit 1))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Provisos for graphical objects
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(tm-define (graphics-get-proviso) (graphics-get-property "gr-proviso"))

(define (graphics-test-proviso? val)
  (with old
    (graphics-get-property "gr-proviso")
    (or (and (func? val 'quasiquote 1) (graphics-test-proviso? (cadr val)))
      (and (pair? val) (pair? old) (== (car val) (car old)))
      (== val old)
    ) ;or
  ) ;with
) ;define
...
(tm-define (graphics-get-pen-enhance-method)
  (with v
    (graphics-get-property "gr-pen-enhance")
...
(tm-define (graphics-set-pen-enhance-method val)
  ...
  (with strength
    (graphics-get-pen-enhance-strength)
    (with v
      (cond ((== strength "1") val)
...
(tm-define (graphics-get-pen-enhance-strength)
  (with v (graphics-get-property "gr-pen-enhance") (if (string? v) "1" (caddr v)))
) ;tm-define
...
(tm-define (graphics-set-pen-enhance-strength val)
  ...
  (with method
    ...
    (with v
      (if (== val "1") method `(tuple ,method ,val))
      (graphics-set-property "gr-pen-enhance" v)
    ) ;with
  ) ;with
) ;tm-define
...
(define (graphics-toggled-property? var)
  (lambda () (!= (graphics-get-property var) "default"))
) ;define
...
(tm-define (graphics-get-snap-distance)
  (with val
    (graphics-get-property "gr-snap-distance")
    (if (string? val) val "10px")
  ) ;with
) ;tm-define

(tm-define (graphics-get-snap type)
  (or (in? type (get-snap)) (in? "all" (get-snap)))
) ;tm-define

(tm-define (graphics-test-snap? type)
  (if (== type "none") (null? (get-snap)) (graphics-get-snap type))
) ;tm-define

(tm-define (graphics-set-snap type)
  (:check-mark "*" graphics-test-snap?)
  (cond ((== type "none") (set-snap (list)))
        ((== type "all") (set-snap (list "all")))
        ((nin? type (get-snap)) (set-snap (cons type (get-snap))))
  ) ;cond
) ;tm-define
...
(define (object-set-property-bis t var val)
  (cond ((tree-is? t :up 'with) (with-set (tree-up t) var val 0))
        ((!= val "default") (tree-set! t `(with ,var ,val ,t)))
  ) ;cond
) ;define
...
(tm-define (object-get-property var) (tree->stree (get-env-tree var)))

(tm-define (object-set-property var val)
  (and-with t
    (tree-innermost graphical-context?)
...
(define (object-toggled-property? var)
  (lambda () (!= (object-get-property var) (get-init var)))
) ;define
