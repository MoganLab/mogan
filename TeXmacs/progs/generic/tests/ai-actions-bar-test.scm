...
(define img '(image "$TEXMACS_PATH/misc/images/new-mogan-128.png" "77pt" "77pt" "" ""))
...
(define (test-only-images)
  ...
  (check (ai-selection-only-images? (tm->tree `(document ,img))) => #t)
  (check (ai-selection-only-images? (tm->tree `(concat ,img ,img))) => #t)
  (check (ai-selection-only-images? (tm->tree `(document " " ,img ""))) => #t)
  ;; 图片夹杂文字/其它内容：照常弹出
  (check (ai-selection-only-images? (tm->tree `(concat ,img "text"))) => #f)
  (check (ai-selection-only-images? (tm->tree `(document ,img "x"))) => #f)
  ;; with 包裹（参数串非空白）：保守判 #f，照常弹出
  (check (ai-selection-only-images? (tm->tree `(with "color" "red" ,img))) => #f)
) ;define
