...
;; 注：图片标记约定的仓库内实现在 (generic ai-actions-bar) 的 ai-flatten-text
;; （0996 释义上下文复用同一约定，但公式/表格那边登记子树、留 <#Z<k>G> 哨兵
;; 以便换回真实子树，纯文本消费方如需公式/表格标记应自行映射回
;; [FORMULA]/[TABLE]）；liii ghost 插件落地时应改为转调它，避免两份标记表
;; 各自漂移。
