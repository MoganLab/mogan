...
(tm-define (semantic-popup-actions raw-tag t)
  (let ((tag (semantic-normalize-tag raw-tag)))
    (cond ((== tag 'equation*)
           '(actions (action "copy-latex" "Copy LaTeX" "tm_copy")
          ...
          ((in? tag '(table-of-contents table-of-contents*))
           '(actions (action "refresh-toc" "Refresh TOC" "tm_reload"))
          ) ;
          (else '(actions))
    ) ;cond
  ) ;let
...
(tm-define (semantic-popup-action-trigger raw-tag action-id t)
  (let ((tag (semantic-normalize-tag raw-tag)))
    (cond ((== action-id "copy-latex") (semantic-copy-latex t))
          ((== action-id "toggle-number") (semantic-toggle-equation-number t))
          ((== action-id "refresh-toc") (update-document "table-of-contents"))
          (else #f)
    ) ;cond
  ) ;let
) ;tm-define
