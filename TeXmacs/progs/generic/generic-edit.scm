...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Algorithm macro enter key navigation: jump to next parameter like right arrow
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (in-algo-macro-param? t)
  (or (and-with parent (find-algo-macro-parent t)
        (let* ((path (cursor-path))
               (parent-path (tree->path parent)))
          (and parent-path
               (> (length path) (length parent-path))
      ...
      (and (tree-in? t algo-macro-tags)
           (let* ((path (cursor-path))
             ...
             (and t-path
                  ...
                  (let ((param-index (list-ref path (length t-path))))
                    (and (integer? param-index)
                         (< param-index (1- (tree-arity t))))))))))
...
(tm-define (kbd-return)
  (:require (in-algo-macro-param? (focus-tree)))
  (with parent (find-algo-macro-parent (focus-tree))
    (let* ((path (cursor-path))
           (parent-path (tree->path parent)))
      (when parent-path
        (let ((param-index (list-ref path (length parent-path))))
