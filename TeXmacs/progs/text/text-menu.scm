...
(tm-define (chapter-number-style-var t)
  (with l (tree-label t)
    (if (in? l '(chapter chapter*)) "chapter-number-style" #f)))
  ...
  (with var (focus-section-title-style-var t)
    (when var
      ...
      ((check "Left aligned" "v" (== (get-init-env var) "left"))
       (init-env var "left"))
      ---))
  (with num-var (chapter-number-style-var t)
    (when num-var
      (group "Number style")
      ((check "Arabic (1, 2, 3)" "v" (== (get-init-env num-var) "arabic"))
       (init-env num-var "arabic"))
      ((check "Chinese (一, 二, 三)" "v" (== (get-init-env num-var) "hanzi"))
       (init-env num-var "hanzi"))
      ((check "Roman (I, II, III)" "v" (== (get-init-env num-var) "Roman"))
