...
(define (make-empty-list-item item)
  (cond ((tree-is? item 'concat)
         `(concat (,(tree-label (tree-ref item 0))) ""))
        (else
         `(,(tree-label item) ""))))

(tm-define (structured-insert-vertical t downwards?)
  (:require (itemize-enumerate-context? t))
  (and-with item (current-list-item-tree)
    (and-with doc (tree-up item)
      (when (tree-is? doc 'document)
        (let* ((idx (tree-index item))
               (new-idx (if downwards? (+ idx 1) idx))
               (new-item-stree (make-empty-list-item item)))
          (tree-insert! doc new-idx (list new-item-stree))
          (with new-item (tree-ref doc new-idx)
            (if (tree-is? new-item 'concat)
                (tree-go-to new-item 1 0)
                (tree-go-to new-item 0 0))))))))

(tm-define (structured-remove-vertical t downwards?)
  (:require (itemize-enumerate-context? t))
  (and-with item (current-list-item-tree)
    (and-with doc (tree-up item)
      (when (and (tree-is? doc 'document)
                 (> (tree-arity doc) 1))
        (let* ((idx (tree-index item))
               (nr (tree-arity doc)))
          (tree-remove! doc idx 1)
          (let ((new-idx (if downwards?
                             (min idx (- nr 2))
                             (max 0 (- idx 1)))))
            (with new-item (tree-ref doc new-idx)
              (if (tree-is? new-item 'concat)
