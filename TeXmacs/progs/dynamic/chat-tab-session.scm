...
(define (chat-tab-add-table-borders-stree x)
  (define (process x in-big-table?)
    (cond ((string? x) x)
          ((list? x)
           (let ((head (car x)))
             (cond ((eq? head 'big-table)
                    `(big-table ,@(map (lambda (c) (process c #t)) (cdr x))))
                   ((eq? head 'tformat)
                    (let loop ((children (cdr x)) (has-border? #f))
                      (if (null? children)
                          (if has-border?
                              `(tformat ,@(map (lambda (c) (process c in-big-table?)) (cdr x)))
                              `(tformat (cwith "1" "-1" "1" "-1" "cell-tborder" "1ln")
                                        ...
                                        ,@(map (lambda (c) (process c in-big-table?)) (cdr x))))
                   ...
                   ((member head '(tabular tabular*))
                    (if in-big-table?
                        `(,head ,@(map (lambda (c) (process c in-big-table?)) (cdr x)))
                        `(big-table (,head ,@(map (lambda (c) (process c #t)) (cdr x)))
                                    (document ""))))
                   (else
                    (cons head (map (lambda (c) (process c in-big-table?)) (cdr x)))))))
          (else x)))
  (process x #f))
