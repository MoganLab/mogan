...
glue_goldfish (s7_scheme* sc) {
  ...
  const char* d_gfproject_load_config=
      "(g_gfproject-load-config) => string, returns merged gfproject.json";
  ...
  s7_define (sc, cur_env, s7_make_symbol (sc, s_gfproject_load_config),
             s7_make_typed_function (sc, s_gfproject_load_config,
                                     f_gfproject_load_config, 0, 0, false,
...
static void
goldfish_print_prefixed_scheme_error_message (s7_scheme*    sc,
                                              const string& prefix,
                                              const char*   errmsg) {
  if ((errmsg) && (*errmsg)) {
    string rendered;
...
gfproject_extract_tools (const json& config) {
  if (config.is_object () && config.contains ("tools") &&
      config["tools"].is_object ()) {
    return config["tools"];
  }
  return json::object ();
}
...
load_gfproject_config_bundle (const char* gf_lib) {
  ...
  json merged=
      bundle.lib_config.is_object () ? bundle.lib_config : json::object ();
  ...
  for (const auto& [command, local_tool] : local_tools.items ()) {
    if (merged_tools.contains (command)) {
      merged_tools[command]=
          gfproject_deep_merge_value (merged_tools[command], local_tool);
    }
    else {
...
static string find_tool_root_by_command (const char*   gf_lib,
                                         const string& command);

static gfproject_tool_prepare_result
goldfish_prepare_tool_main (s7_scheme* sc, const char* gf_lib,
                            const string& command, const json& tool_config) {
  ...
  if (!tool_config.is_object ()) {
    ...
    result.message=
        "Error: Tool '" + command + "' config must be a JSON object.";
    return result;
  }

  if (!tool_config.contains ("organization") ||
      !tool_config.contains ("module")) {
  ...
  if (!tool_config["organization"].is_string () ||
      !tool_config["module"].is_string ()) {
    result.error= gfproject_tool_prepare_error::invalid_config_value;
    result.message=
        "Error: Tool '" + command + "' organization/module must be strings.";
    return result;
  }
  ...
  if (tool_root.empty ()) {
    ...
    result.message=
        "Error: tools/" + command + "/" + org + " directory not found.";
    return result;
  }
  ...
  if ((!main_func) || (!s7_is_procedure (main_func))) {
    ...
    result.message=
        "Error: Failed to find main function in (" + org + " " + module + ").";
    return result;
  }
...
static int
goldfish_finish_tool_error (s7_scheme* sc, const string& message,
                            const char*& errmsg, s7_pointer old_port,
                            int gc_loc, bool include_scheme_error) {
...
static int
goldfish_finish_tool_success (s7_scheme* sc, s7_pointer result,
                              const char*& errmsg, s7_pointer old_port,
                              int gc_loc) {
  errmsg= s7_get_output_string (sc, s7_current_error_port (sc));
  goldfish_print_scheme_error_message (sc, errmsg);
...
static int
goldfish_run_tool_with_config (s7_scheme* sc, const char* gf_lib,
                               const string& command, const json& tool_config,
                               const char*& errmsg, s7_pointer old_port,
                               int gc_loc, bool allow_fallback) {
  gfproject_tool_prepare_result prepared=
      goldfish_prepare_tool_main (sc, gf_lib, command, tool_config);
  if (prepared.error != gfproject_tool_prepare_error::none) {
    ...
    bool include_scheme_error=
        prepared.error == gfproject_tool_prepare_error::import_failed;
    return goldfish_finish_tool_error (sc, prepared.message, errmsg, old_port,
...
goldfish_run_tool (s7_scheme* sc, const char* gf_lib, const string& command,
  ...
  bool allow_builtin_fallback= command == "help" || command == "version" ||
                               command == "eval" || command == "load" ||
                               command == "repl" || command == "run";

  if (resolved.has_local_override && resolved.has_lib_tool) {
    int merged_ret= goldfish_run_tool_with_config (sc, gf_lib, command,
                                                   resolved.merged_tool, errmsg,
                                                   old_port, gc_loc, true);
    if (merged_ret != -1) {
      return merged_ret;
    }
    return goldfish_run_tool_with_config (sc, gf_lib, command,
                                          resolved.lib_tool, errmsg, old_port,
                                          gc_loc, allow_builtin_fallback);
  }

  return goldfish_run_tool_with_config (sc, gf_lib, command,
                                        resolved.merged_tool, errmsg, old_port,
                                        gc_loc, allow_builtin_fallback);
}
...
extract_library_name_from_form (s7_scheme* sc, s7_pointer library_name_form,
  ...
  if ((!s7_is_list (sc, library_name_form)) ||
      (s7_list_length (sc, library_name_form) != 2)) {
    return false;
  }
...
make_library_name_list (s7_scheme* sc, const string& group,
  ...
  return s7_list (sc, 2, make_library_name_part (sc, group),
                  make_library_name_part (sc, library));
}
...
f_function_libraries (s7_scheme* sc, s7_pointer args) {
  ...
  try {
    visible_library_queries=
        find_function_libraries_in_load_path (sc, function_name);
    ...
    return s7_error (
        ...
        s7_list (
            ...
            s7_make_string (
                sc,
                (string (
                     "g_function-libraries: failed to inspect libraries: ") +
                 ex.what ())
                    .c_str ()),
            function_name_arg));
  }
...
repl_for_community_edition (s7_scheme* sc, int argc, char** argv) {
  ...
  if (gfproject_config.contains ("tools") &&
      gfproject_config["tools"].contains (command)) {
    int tool_ret=
    ...
    // If tool_ret == -1, tool config exists but execution failed, fall through
    // to check other commands
  }
