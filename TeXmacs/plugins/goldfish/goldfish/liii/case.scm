                ...
                (let ((matcher ((funclet ((funclet 'case*) 'case*-helper)) 'handle-sequence)))
                  (or (equivalent? matchee pattern)
                    (and (or (pair? matchee) (vector? matchee))
                      (begin
                        (fill!
                         ((funclet ((funclet 'case*) 'case*-helper)) 'labels)
                         #f
                        ) ;fill!
                    ...
                    (let ((func
                      ...
                      (if (pair? pat)
                        (cond
                         ...
                         (else
                           (let ((new-pat (make-list new-pat-len)) (new-sel (make-list new-pat-len)))
                             (if ellipsis-label
                               (set! (labels ellipsis-label)
                        ...
                        (cond
                         ...
                         (else
                           (let ((new-pat (make-vector new-pat-len)) (new-sel (make-vector new-pat-len)))
                             (if ellipsis-label
                               (set! (labels ellipsis-label)
                                 (list 'quote (copy sel (make-list (- sel-len new-pat-len)) pos))
                               ) ;set!
                    ...
                    (let ((str (substring str1 2 str1-end)))
                      (if (= (length str) 0)
                        ...
                        (let ((colon (char-position #\: str)))
                          (cond
                            (colon
                              (let ((label (substring str 0 colon)) (func (substring str (+ colon 1))))
                                (cond
                                 ((labels label)
                                  (lambda (sel)
                                    (error 'syntax-error
                      ...
                      (let ((func-ok #t))
                        (when (or (pair? pat) (vector? pat))
                          ...
                          (let ((pos (if (pair? pat)
                            ...
                            (when
                              (and pos (>= (length sel) (- (length pat) 1)))
