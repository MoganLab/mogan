...
(tm-define (tmhtml-initialize opts)
  ...
  (set! tmhtml-base64?
        (not (== (get-preference "texmacs->html:base64") "off")))
...
(tm-define (tmhtml-read-binary-file file-url)
  (let* ((path (url->string (url-concretize file-url)))
    ...
    (if (not port)
        #u8()
        (let* ((bytes (let loop ((c (read-char port)) (res '()))
                        (if (eof-object? c)
                            (reverse res)
    ...
    (with cached (ahash-ref tmhtml-image-cache x)
      (if (not cached)
          (receive (name-url name-string) (tmhtml-image-names "png")
            ...
            (set-message (string-append "Exporting inline image " (number->string tmhtml-image-progress) " ...") "HTML Export")
...
(tm-define (tmhtml-extract-embedded name)
  ...
  (if (and (func? name 'tuple 2) (string? (caddr name)))
      (let* ((raw (cadr name))
             ...
             (suffix (caddr name)))
        (cond ((symbol? data)
               (cons (tmhtml-symbol-hex->byte-vector data) suffix))
              ((string? data)
...
(define (tmhtml-image l)
  ...
  (cond ((and (string? (car l)) (in? (tmhtml-image-suffix (car l)) (list "ps" "eps" "pdf" "tif")))
         (tmhtml-png (cons 'image l)))
        ((tmhtml-extract-embedded (car l))
         (with embedded (tmhtml-extract-embedded (car l))
           (let* ((data (car embedded))
                  (ext (cdr embedded)))
             (receive (name-url name-string) (tmhtml-image-names ext)
               (let* ((abs-url (url-concretize name-url))
                      (abs-string (url->unix abs-url)))
                 (tmhtml-write-binary-file abs-url data)
                 (with res (tmhtml-image (cons abs-string (cdr l)))
