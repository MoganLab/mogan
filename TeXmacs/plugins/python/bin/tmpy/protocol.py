...
def texmacs_escape(data):
    return data.replace(
        DATA_BEGIN.encode(), (DATA_ESCAPE + DATA_BEGIN).encode()
...
def flush_any(out_str):
    ...
    Output results back to TeXmacs, with the DATA_BEGIN, DATA_END
    control characters.
    """
    data_begin()
...
def get_plugin_path(name):
    the_home_path = (
        os.getenv("TEXMACS_HOME_PATH").replace("\\", "/") + "/plugins/" + name
    )
    the_sys_path = (
        os.getenv("TEXMACS_PATH").replace("\\", "/") + "/plugins/" + name
    )
