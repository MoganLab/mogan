...
reverse (int& r, int& g, int& b) {
  ...
  if (M != m) {
    double lambda1=
        max (((double) (t - m)) / t, ((double) (M - t)) / (255 - t));
    double lambda2=
        max (((double) (t - m)) / tt, ((double) (M - t)) / (255 - tt));
    mu= lambda1 / lambda2;
  }
  r= (int) (tt + mu * (r - t) + 0.5);
  g= (int) (tt + mu * (g - t) + 0.5);
...
rgb_color (int r, int g, int b, int a) {
  ...
  else if ((r == g) && (g == b)) return (a << 24) + ((r * GREYS + 128) / 255);
  else {
    r= (r * CSCALES + 128) / 255;
    g= (g * CSCALES + 128) / 255;
    b= (b * CSCALES + 128) / 255;
...
rgb_color (array<int> rgba) {
  return rgb_color (
      N (rgba) >= 1 ? rgba[0] : 255, N (rgba) >= 2 ? rgba[1] : 255,
      N (rgba) >= 3 ? rgba[2] : 255, N (rgba) >= 4 ? rgba[3] : 255);
}
...
get_hex_color (color c) {
  ...
  if (a == 255)
    return "#" * as_hexadecimal (r, 2) * as_hexadecimal (g, 2) *
           as_hexadecimal (b, 2);
  else
    return "#" * as_hexadecimal (r, 2) * as_hexadecimal (g, 2) *
           as_hexadecimal (b, 2) * as_hexadecimal (a, 2);
}
