 ...
 *            标签走 moebius 的 name<->op 注册表：导出用 as_string(tree_label) 取名字，
 *            导入用 make_tree_label(name) 反查（含扩展标签）。generic 节点（op<0）
 *            用十进制存取（as_string 支持 int、as_int 支持负号），保证忠实还原。
 *
 *            本文件不依赖 loro-c。后续 loro.cpp 的 FFI 薄层只需把 LoroIRNode <-> LoroDoc 字节。
 *  \author Jim Zhou
 *  \date   2026
 */
