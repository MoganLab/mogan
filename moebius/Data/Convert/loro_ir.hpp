...
/** @brief LoroIRNode -> moebius tree（深度复制）。按值传递：lolly 的 string/array
 *  是 COW 句柄，且其 operator()/operator[] 无 const 重载，按值传可让形参可变。 */
tree loro_ir_to_tree (loro_ir_node node);

/**
 * @name 扁平二进制编解码（与 3rdparty/mogan-loro-ffi/src/lib.rs 的格式逐字节一致）
 *
 * 格式（小端）：
 *   node := kind:u8  label_len:u32 label:bytes  text_len:u32 text:bytes
 *           n_children:u32  node × n_children
 * 暴露出来便于在 loro_ir_test 里独立验证 C++ 编解码（不依赖 LORO_ENABLED/Rust）。
 */
///@{
