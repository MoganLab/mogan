 ...
 *  \copyright GPLv3
 *  \details Phase 2 shadow LoroDoc：每个编辑器挂一个 live LoroDoc，随本地编辑逐操作镜像。
 *            seed 用文档树构建 live doc 并填充 rep*→TreeID 身份表（供 Step 4 镜像编辑用）。
 *            仅 LORO_ENABLED 下存在；关闭时不编译，无 Rust 依赖。
 *  \author Jim Zhou
 *  \date   2026
 */
...
public:
   ...
   *  其余 mod（结构、SPLIT/JOIN、ASSIGN 等）暂走整树重 seed 兜底，保证 shadow 始终与
   *  buffer 一致——后续逐步把这些也精确化。 */
  void   mirror_mod (tree doc_root, modification mod);
  /** @brief live doc -> snapshot 字节（包成 lolly string）。 */
  string export_snapshot ();
  /** @brief 把 snapshot/update 字节 import（合并）进 live doc。Phase 3 反向同步。 */
  bool import_data (string bytes);
  /** @brief import 后用"带 TreeID 的增强 IR"重建 buffer 并把 buffer rep 关联到导入的
   *         TreeID（填 id_map），使本端后续编辑能与对端合并。out_buffer 即新的 buffer。 */
  bool import_and_build (string bytes, tree& out_buffer);
  /** @brief 订阅本地编辑产生的增量 update：每次本地 op 提交时回调。事件级增量同步。 */
  void on_local_update (mogan_local_update_cb cb, void* user_data);
  /** @brief 把远端 update 导入后，diff buffer（旧）vs shadow（新），生成把 buffer 变到
   *         新状态所需的 modification 序列（文本字符级 diff → INSERT/REMOVE；结构差异 →
   *         整树 ASSIGN 兜底）。供编辑器在 versioning 模式下经 edit_modify 应用到 buffer。 */
  list<modification> remote_diff_mods (string bytes, tree buffer);
  /** @brief diff buffer（旧）vs shadow 当前状态（新），生成把 buffer 变到 shadow 所需的
   *         modification（不 import，用于 debug_loro 本地 round-trip）。 */
