 ...
 *            - 开启：调用真实 FFI（mogan_loro_encode/decode），链 mogan-loro-ffi。
 *            - 关闭（默认）：tree_to_loro/loro_to_tree 为空桩，返回空，无 Rust 依赖，
 *              保证主构建不被未接好的 FFI 拖垮。
 *  \author Jim Zhou
 *  \date   2026
 */
...
/** @brief body 树 -> Loro CRDT snapshot 字节（包成 lolly string）。失败返回空串。 */
 ...
 *
 * 仅在 LORO_ENABLED 下声明。对应 3rdparty/mogan-loro-ffi/src/lib.rs 的 extern "C"。
 *****************************************************************************/
#ifdef LORO_ENABLED
#include <cstdint>
#include <cstddef>
...
/// local-update 事件回调：本地编辑产生增量 delta 时回调 cb(user_data, bytes, len)
typedef void (*mogan_local_update_cb) (void* user_data, const uint8_t* bytes, size_t len);
...
int32_t       mogan_loro_doc_to_ir_with_ids (void* doc, uint8_t** out, size_t* out_len);
...
int32_t       mogan_loro_node_set_label (void* doc, mogan_tree_id id, const uint8_t* label,
                                         size_t len);
int32_t       mogan_loro_node_set_binary (void* doc, mogan_tree_id id, const uint8_t* bytes,
                                          size_t len);
