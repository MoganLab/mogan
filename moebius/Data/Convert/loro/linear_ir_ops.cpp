...
//   复合 -> 其 OPEN；原子 -> 其 TEXT/BINARY；target 为 nil -> 根。未找到返回 -1。
static int
...
static void
push_range (array<linear_item>& dst, array<linear_item>& src, int from, int len) {
  for (int i= 0; i < len; i++)
    dst << src[from + i];
}
...
linear_ir_apply_mod (array<linear_item> items, modification mod) {
  switch (mod->k) {
  case MOD_SPLIT: return apply_split (items, mod);
  case MOD_JOIN: return apply_join (items, mod);
  case MOD_INSERT_NODE: return apply_insert_node (items, mod);
  case MOD_REMOVE_NODE: return apply_remove_node (items, mod);
  default: return items; // ASSIGN 等：本次不动
  }
}
