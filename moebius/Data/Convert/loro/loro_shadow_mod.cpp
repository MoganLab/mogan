 ...
 *  \date   2026
 *
 * 一个 modification 镜像到 body 单 LoroText（markup 流）。优先走 compute_markup_edit
 * 给出的最小字节 splice（文本增删、SPLIT/JOIN——存活字符 op-id 不变）；不可精确
 * 翻译的（ASSIGN / INSERT_NODE / REMOVE_NODE / 复合子树增删）退化为整 body 重 seed，
 * 保证 shadow 始终与 buffer 一致。
 */
...
loro_shadow_rep::mirror_mod (tree doc_root, modification mod) {
  ...
  if (ed.ok) {
    ...
    if (ed.delete_len > 0)
      mogan_loro_body_text_delete (doc, (size_t) ed.offset, (size_t) ed.delete_len);
