 ...
 *            一致，可用
 *              linear_ir_to_tree(linear_ir_apply_mod(ir, mod)) == clean_apply(tree, mod)
 *            校验。
 *
 *            编辑形式（均在 item 序列上插入/删除，不重建存活内容）：
 *            - SPLIT 原子：一个 TEXT 拆成 TEXT(head)+TEXT(tail)（markup 层即 SEP 边界）。
 *            - SPLIT 复合：在切分点插入 CLOSE + OPEN(同 label)，原子树一字不动。
 *            - JOIN  原子：合并两个相邻 TEXT 为一个。
 *            - JOIN  复合：删除中间的 CLOSE + OPEN 边界。
 *            - INSERT_NODE：用 OPEN/CLOSE 包裹目标，并把 wrapper 的既有子节点就位。
...
array<linear_item> linear_ir_apply_mod (array<linear_item> items, modification mod);
