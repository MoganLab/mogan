...
// body 不再用 LoroTree，而是一条名为 "body" 的 LoroText，承载整篇 body 的 markup
// （linear_ir ↔ markup，见 linear_ir*.cpp）。偏移均为 utf-8 字节坐标。
int32_t mogan_loro_body_seed (void* doc, const uint8_t* markup, size_t len);
int32_t mogan_loro_body_text_insert (void* doc, size_t pos, const uint8_t* bytes,
                                     size_t len);
