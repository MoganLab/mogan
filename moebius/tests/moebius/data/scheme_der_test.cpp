...
TEST_CASE ("scm_quote escapes quotes and backslashes") {
  CHECK (scm_quote ("plain") == "\"plain\"");
  CHECK (scm_quote ("a\"b") == "\"a\\\"b\"");
  CHECK (scm_quote ("a\\b") == "\"a\\\\b\"");
  CHECK (scm_quote ("") == "\"\"");
  CHECK (scm_quote ("mix\\\"end") == "\"mix\\\\\\\"end\"");
}
...
TEST_CASE ("slash link probe") { CHECK (moebius::data::slash ("x") == "x"); }

TEST_CASE ("slash escapes specials and control chars") {
  CHECK (moebius::data::slash ("plain") == "plain");
  CHECK (moebius::data::slash ("a b") == "a\\ b");
  CHECK (moebius::data::slash ("(x)") == "\\(x\\)");
  CHECK (moebius::data::slash ("a\\b") == "a\\\\b");
  CHECK (moebius::data::slash ("a\tb\nc") == "a\\tb\\nc");
  CHECK (moebius::data::slash (string ("a\0b", 3)) == "a\\0b");
  // 已带引号的串中特殊字符不再转义
  CHECK (moebius::data::slash ("\"a b\"") == "\"a b\"");
}

TEST_CASE ("slash roundtrip with unslash via parser") {
  // slash 后的词元可被解析器还原成单一原子
