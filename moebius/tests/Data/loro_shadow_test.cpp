 ...
 *            to_tree / export_snapshot(→loro_to_tree) 都与原树深度相等，且身份表覆盖各节点。
 *            仅 LORO_ENABLED 下编译用例。
 *  \author Jim Zhou
 *  \date   2026
 */
...
TEST_CASE ("loro_shadow: mirror text typing and backspace") {
  ...
  // 敲 "abc"：只喂 mod 给 mirror_mod（它读 mod 不读树状态），再手工把期望结果赋给 t
...
// 每步都用期望的真值 buffer 比对 shadow.to_tree()，证明 shadow 始终与 buffer 一致。
...
// 双向合并 e2e：A/B 从公共初始 snapshot 各自并发编辑不同节点，互导后收敛到同一棵树。
// 关键：B 用 import_and_build 把 buffer rep 关联到 A 的 TreeID，二者身份对齐才能合并。
...
// 事件级增量同步：A 订阅 local-update，编辑产生 delta（仅新增 op）→ B 导入 delta 收敛。
extern "C" void
...
// 远端 delta → modification：B 收到 A 的编辑后，生成把 buffer 变到新状态所需的 mod，
// 手工应用到 buf（模拟 edit_modify），验证 buf 与 A 的编辑结果一致。
TEST_CASE ("loro reverse: remote edit -> modification (apply to buffer)") {
  ...
  for (list<modification> l= mods; !is_nil (l); l= l->next) {
    ...
    if (m->k == MOD_INSERT) {
      ...
      string s   = m->t->label;
      int    pos = index (m);
      atom->label= atom->label (0, pos) * s * atom->label (pos, N (atom->label));
    }
    else if (m->k == MOD_REMOVE) {
      tree& atom= subtree (buf, root (m));
      int   pos = index (m);
      int   nr  = argument (m);
      atom->label= atom->label (0, pos) * atom->label (pos + nr, N (atom->label));
    }
