...
TEST_CASE ("markup_edit: ASSIGN compound subtree is precise") {
  ensure_labels ();
...
TEST_CASE ("markup_edit: ASSIGN whole body (path ()) is precise") {
  ensure_labels ();
  ...
  neu[0]          = tree ("new1");
  neu[1]          = tree ("new2");
  modification mod= mod_assign (path (), neu);
  CHECK_EQ (markup_edit_roundtrip (t, mod), true);
  markup_edit ed= compute_markup_edit (tree_to_linear_ir (t), mod);
  CHECK_EQ (ed.ok && N (ed.ops) == 1, true);
}

TEST_CASE ("markup_edit: ASSIGN_NODE relabel is precise") {
  ensure_labels ();
  // (document (para "x")) —— 把 para 的标签改成 concat
  tree t (DOCUMENT, 1);
  t[0]            = tree (PARA, 1);
  t[0][0]         = tree ("x");
  modification mod= mod_assign_node (path (0), CONCAT);
  CHECK_EQ (markup_edit_roundtrip (t, mod), true);
  markup_edit ed= compute_markup_edit (tree_to_linear_ir (t), mod);
  CHECK_EQ (ed.ok, true);
}
