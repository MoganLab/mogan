...
TEST_CASE ("linear_ir_ops: INSERT_NODE matches clean_apply") {
  ...
  tree t (DOCUMENT, 1);
  t[0]   = tree ("hello");
  CHECK_EQ (apply_matches_clean (t, mod_insert_node (path (0), 0, tree (CONCAT, 0))),
            true);
}
...
markup_edit_roundtrip (tree t, modification mod) {
  ...
  string expected= linear_ir_to_markup (tree_to_linear_ir (clean_apply (t, mod)));
  if (result != expected) {
...
TEST_CASE ("markup_edit: SPLIT atomic inserts only CLOSE+OPEN('')") {
  ...
  CHECK_EQ (markup_edit_roundtrip (t, mod), true);
  markup_edit ed= compute_markup_edit (tree_to_linear_ir (t), mod);
  CHECK_EQ (ed.delete_len == 0 && ed.insert_bytes == close_open_empty_bytes (), true);
}
...
TEST_CASE ("markup_edit: INSERT_NODE falls back to coarse (ok=false)") {
  ...
  t[0]   = tree ("hello");
  markup_edit ed=
      compute_markup_edit (tree_to_linear_ir (t),
                           mod_insert_node (path (0), 0, tree (CONCAT, 0)));
