...
TEST_CASE ("linear_ir_ops: SPLIT compound matches clean_apply") {
  ensure_labels ();
  // (concat (concat "a" "b" "c")) —— 切 root 的 child 0（内层 concat）在 child 1
  tree inner (CONCAT, 3);
  inner[0]   = tree ("a");
  inner[1]   = tree ("b");
  inner[2]   = tree ("c");
...
TEST_CASE ("linear_ir_ops: SPLIT at boundary (at == N(children)) matches clean_apply") {
  ensure_labels ();
  tree inner (CONCAT, 2);
  inner[0]   = tree ("a");
  inner[1]   = tree ("b");
...
TEST_CASE ("linear_ir_ops: INSERT_NODE then REMOVE_NODE round-trips") {
  ...
  // wrapped: (document (concat "hello"))；脱去 child 0(child0 of document) 的 concat
