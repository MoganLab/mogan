 ...
 *            tmu_document_to_tree -> tree_to_loro -> loro_to_tree，断言与原树深度相等。
 *            覆盖真实文档里的数学、表格、图片(raw-data)、样式等结构，暴露映射盲区。
 *
 *            仅在 LORO_ENABLED（xmake option libloro=y）下编译用例；关闭时为空二进制。
 *  \author Jim Zhou
 *  \date   2026
 */
...
// 定位 tmu 夹具目录：尝试常见 CWD 候选（xmake r 默认在项目根）
static string
resolve_fixture_dir () {
  const char* cands[]= {
    "TeXmacs/tests/tmu/",
...
TEST_CASE ("loro real: round-trip all tmu fixtures") {
  ...
  if (N (dir) == 0) {
    MESSAGE ("找不到 TeXmacs/tests/tmu 夹具目录，跳过（请从项目根目录运行）");
    return;
  }
  ...
  for (int i= 0; i < N (names); i++) {
    ...
    // 解析失败会返回 (ERROR "bad format or data")；跳过不计，避免把 error 树的恒等
    // round-trip 误当通过。若全部都 ERROR，下方 tested>0 会兜住（提示 init 不足）。
  ...
  cout << "loro real: tested=" << tested << " passed=" << passed << " failed="
       << failed << " skipped(parse-error)=" << skipped << LF;
