...
### 8.8 缩略图本地缓存（StartupBridge.cpp/hpp）

- **根因**：QML `Image` 对 HTTP URL 默认异步加载，即使 delegate 不重建，切换分类也会因 `source` 变化触发异步重新加载。
- **修复**：`StartupBridge` 增加 `QNetworkAccessManager` + `thumbnailLocalCache_`（remote URL → local file path）。首次加载时异步下载缩略图到 `$TEXMACS_HOME_PATH/system/thumbnails/`，下载完成后将 `thumbnailUrl` 替换为 `file://` 本地路径。后续 QML `Image` 加载 `file://` URL 是同步的（Qt 默认行为），不再闪烁。
- **涉及方法**：`ensureThumbnailsLocal()`、`onThumbnailDownloaded()`、`thumbnailCachePath()`
