...
2. 在 `edit_repaint.cpp` 中的 `repaint_table_handles` 函数中添加判断，如果当前表格所在环境的 `MODE` 环境变量值为 `"math"`，则不显示handles；如果 `TABLE_HANDLES` 环境变量值为 `"false"`，则始终不显示handles
