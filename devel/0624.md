...
6. **真实的量子 Notes 导出转译验证**：引入了真实的物理笔记文档 `0624_quantum.tmu`，编写了专属的 `test-quantum-latex-export` 单元测试。该测试全自动运行 LaTeX 导出，载入转译结果，并利用严格的断言验证物理数学算符（如 `\hbar`（普朗克常数）、`\psi`（波函数）、`\langle` 与 `\rangle`（狄拉克算符）、`\nabla`（哈密顿梯度算符））被 100% 精确转译。同时验证在图片数量为 `0` 的情况下，系统判断精准，绝不画蛇添足触发任何进度条对话框（利用 `resolve-module` 直接对内部全局变量进行隔离校验）。
