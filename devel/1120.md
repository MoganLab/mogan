...
将 `TeXmacs/progs/texmacs/texmacs/tm-files-test.scm` 由 `regression-test-group` 改为 `(liii check)`：
- `(import (liii check))` + `(check-set-mode! 'report-failed)`
- 两个原 case 拆为 `test-auto-backup-official-url`、`test-auto-backup-texmacs-path-buffer?`，用 `(check ... => ...)` 断言
- 入口 `regtest-tm-files` 依次调用两个子测试后执行 `(check-report)`
- 验证：`xmake b tm-files-test && xmake r tm-files-test` → `2 correct, 0 failed`
