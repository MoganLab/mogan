...
构建 Mogan 并启动后，通过 `silent-feed*` 向 telemetry 插件发送 JSON，检查 `$TEXMACS_HOME_PATH/telemetry.log` 是否按预期追加内容：
...
最终效果：`$TEXMACS_HOME_PATH/telemetry.log` 会追加收到的 JSON 字符串。
...
实现一个 fake telemetry 插件，仅用于接收 Mogan 主进程传入的 JSON 数据，并将 JSON 字符串追加写入 `$TEXMACS_HOME_PATH/telemetry.log`。
...
- `goldfish/tm-telemetry.scm` 启动时先 `flush-verbatim "telemetry"` 完成握手；随后循环调用 `read-paragraph-by-visible-eof` 读取主进程发来的段落。读到的内容可能是 `(document "...")` 或直接字符串，统一用 `document->string` 提取。日志路径通过 `(path-join (path-from-env "TEXMACS_HOME_PATH") "telemetry.log")` 获取，提取结果追加到该日志文件，写完后回显确认消息，等待下一次通信。

- 为保持简单，不单独拆出 `(liii telemetry)` 库；写日志与数据提取逻辑直接放在 `tm-telemetry.scm` 中。
