...
`invariants` 的 Doxygen 注释：明确归一化弧长参数定义（到起点弧长 ÷ 总弧长，
[0,1]）、`access` t 为绝对弧长、段表扫描插值实现、边界行为及两者配套关系；
`invariants` 按 learn/recognize 真实使用场景展开（disc/cont 各自内容与角色、
平移缩放不变性来源、两级识别策略、追加写入语义、21 点定长采样契约）。

## 追加：invariants 单元测试与 benchmark（2026-08-19）

`poly_line_test.cpp` 补 3 个用例（共 18 全绿）：level 2 形状（disc 仅轮廓数、
cont 仅 21 点坐标）；平移 + 整体放大不变性（cont 逐点一致）；多轮廓 disc
结构（条数 + 每折线顶点数）与 out 参数追加写入语义。

`poly_line_bench.cpp` 补 `invariants(level=2)` 与
`legacy invariants(level=1)`（旧版：21 点各调一次全扫描 access +
legacy vertices）：

| 项 | 优化版 | legacy 对照 |
|---|---|---|
| invariants(level=1) | 22.9 µs | 119.5 µs（~5.2×）|

（handwriting_bench 中的 invariants 单项历史数据 135 µs → ~10 µs 为
单折线 64 点语料；本表为双折线 50+30 点，量级不同。）
