...
## 追加：access 单项 benchmark（2026-08-19）

`moebius/bench/Kernel/Types/poly_line_bench.cpp` 补充 `access` 的显式基准与
legacy 对照组（旧实现：逐段扫描、每步一次 `l2_norm`、段长对每个 t 重复重算），
另加「沿折线均匀取 21 点」的批量形态，模拟 `invariants` 采样对 `access`
的调用方式。50 顶点折线、t=60：

| 项 | 优化版 | legacy 对照 |
|---|---|---|
| 单次 access | 380 ns | 193 ns |
| access ×21 均匀采样 | 8.77 µs | 10.75 µs（~18%↓）|
...
- moebius/bench/Kernel/Types/poly_line_bench.cpp（新增 access 基准与 legacy 对照）
