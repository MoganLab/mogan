...
### 如何测试
1. 确保 GitHub Repository 已配置以下 Secrets：
   - `APPLE_CERTIFICATE_P12`、`APPLE_CERTIFICATE_PASSWORD`
   - `APPLE_API_KEY_ID`、`APPLE_API_KEY_P8`、`APPLE_API_ISSUER_ID`、`APPLE_TEAM_ID`

2. 推送 tag 或手动触发 `CD for Mogan STEM on macOS arm64` 工作流

3. 验证结果：
   - 检查工作流日志中签名、公证、装订步骤是否成功
   - 下载 DMG，在 macOS 上双击打开，确认没有 "无法验证开发者" 警告
   - 使用 `spctl -a -t open --context context:primary-sil -v <path-to-dmg>` 验证公证状态
   - 使用 `codesign -dvv <path-to-app>` 验证签名详细信息

## 2026/04/23 工作流集成苹果开发者代码签名
### What
1. **CD 工作流集成苹果开发者代码签名**：
   - 在 `cd_on_macos_arm64.yml` 中新增证书导入、应用签名、DMG 签名、公证和装订步骤
   - 本地打包保留 Ad-hoc 签名（`xmake.lua` 中 `codesign --force --deep --sign -`）

2. **签名流程**：
   - 使用 `Developer ID Application` 或 `Apple Distribution` 证书对 `.app` bundle 进行深度签名
   - 先签名内部 `.dylib`、Framework 二进制文件、PlugIns，最后签名整个 bundle
   - `stem_packager` 打包后重新挂载 DMG 并再次签名，解决签名丢失问题

3. **公证与装订**：
   - 使用 App Store Connect API Key 通过 `xcrun notarytool submit` 提交 DMG 公证
   - 成功后使用 `xcrun stapler staple` 装订公证票据
   - 装订失败时重试 3 次，不强制中断流程
