...
## 任务 4：kernel 阶段细化打点 + 移除 kernel/old-gui 死代码

### What

- kernel 阶段的单一 `bench kernel` 打点拆为子阶段：`prelude`（文件头部
  telemetry/load/eval/catch 覆盖）、`kernel/boot-s7`、`kernel/boot`、
  `kernel/library`、`kernel/regexp`、`kernel/logic`、`kernel/texmacs`、
  `kernel/gui`，按 inherit-modules 调用边界打点。
- 移除 `TeXmacs/progs/kernel/old-gui/` 四个模块（widget/factory/form/test）
  及 init-research.scm 的 inherit 条目。

### Why

kernel 是启动第一大阶段（130+ ms），单一打点无法定位回归子模块。
old-gui 是 2007 年遗留的 tm-build widget 构造体系，全仓（scheme + C++）
除 init-research.scm 的加载外无任何引用（入口宏 tm-build*/interactive-form/
widget-call-back 等均零调用），已被 gui-markup/menu-define 体系取代，
属死代码，移除可省 kernel/gui 阶段加载开销。

### 验证

`xmake r 1194` 启动正常，打点输出（本机一轮）：
...
| kernel/regexp | 6 ms |
| kernel/logic | 8 ms |
| kernel/texmacs | 52 ms |
| kernel/gui | 49 ms（移除 old-gui 后） |

kernel/texmacs 与 kernel/gui 为 kernel 内两大子阶段，后续可继续下钻。

### 涉及文件

- `TeXmacs/progs/init-research.scm`
- 删除 `TeXmacs/progs/kernel/old-gui/`（4 个 .scm）
