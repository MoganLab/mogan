...
## 5 What
`exec_use_package`（`env_exec.cpp`）：本地优先解析，支持从当前文档 buffer（`base_file_name`）所在目录相对解析带路径的样式包（如 `beamer/daxue_beamer`），未命中再回退到 `$TEXMACS_STYLE_PATH` 与 `resolve_dotted_package`。
