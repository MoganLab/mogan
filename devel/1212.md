...
- `moebius/tests/Kernel/Types/point_test.cpp`：混维度守卫单元测试，
  构造 `N(p1)=100, N(p2)=34, N(p3)=100`（`i` 容量 64、循环跑到 100），
  ASan 构建下测试自身即可检出 `heap-buffer-overflow READ of size 8
  at point.cpp:278`；常规构建下守卫结果维度与数值。
...
- **验证**：
  - ASan 复现（独立编译 point.cpp + 混维度 main）：修复前报
    `heap-buffer-overflow READ of size 8 at point.cpp:278`，
    修复后干净退出且输出 `N(i)=34, N(j)=34`；
