...
在 commit `9deb90253c` 的优化改动中，`orthogonalize` 与 `midperp` 在处理不同维度的点时存在堆缓冲区越界读（heap-buffer-overflow）：

1. **`orthogonalize` 越界读**：
   `n = min(N(p2), N(p1))` 分配 `i` 数组；
   后续循环以 `m = min(N(p3), N(p1))` 驱动：
   `c += d[k] * i[k];` 及 `j[k] = d[k] - c * i[k];`
   当 `m > n` 时，`i[k]` 越界读取未分配的内存。
2. **`midperp` 越界读**：
   `j` 的维度为 `m = min(N(p3), N(p1))`；
   循环按 `n = min(N(p1), N(p2))` 跑：
   `a.p1[k] = a.p0[k] + j[k];`
   当 `m < n` 时，`j[k]` 越界读取未分配的内存。

### 复现物（来自 `da/1265/stage` 分支）

- `devel/1265_3.cpp`：ASan 独立复现程序（构造 `N(p1)=100, N(p2)=34, N(p3)=100`，ASan 明确报出 `heap-buffer-overflow READ of size 8 at point.cpp:278`）。
- `moebius/tests/Kernel/Types/point_test.cpp`：混维度行为守卫单元测试。

### 修复（`da/1212/fix`）

- **What**：消除 `orthogonalize`/`midperp` 混维度输入下的越界读。
- **Why**：优化时把原实现中 `operator-`/`operator+` 逐分量取 `min` 的
  维度语义丢了，导致循环维度与数组分配维度不一致。
- **How**：
  1. `orthogonalize`：`j` 相关循环的维度由 `m = min(N(p3), N(p1))` 改为
     `m = min(n, N(p3))`（即三者最小），恢复优化前
     `(p3-p1) - inner(p3-p1,i)*i` 的逐分量 min 语义；
  2. `midperp`：`a.p1 = a.p0 + j` 逐分量取 `min(n, N(j))`，
     `a.p1` 维度为 `min(n, N(j))` 而非 `n`。
- **涉及文件**：
  - `moebius/Kernel/Types/point.cpp`（修复）
  - `moebius/tests/Kernel/Types/point_test.cpp`（混维度守卫用例更新：
    `orthogonalize` 断言 `N(j)==34`；`midperp` 新增 `m<n` 方向用例）
  - `devel/1265_3.cpp`（ASan 复现程序，随本分支留存）
- **验证**：
  - ASan 复现程序：修复前报 `heap-buffer-overflow READ of size 8 at
    point.cpp:278`，修复后干净退出且输出 `N(i)=34, N(j)=34`；
  - `xmake test moebius_tests/point_test` 通过；moebius 全量测试通过
    （`shared_lib_test` 失败为干净树上已存在的无关失败）；
  - `xmake b stem` 链接通过。
