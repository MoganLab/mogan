...
使用 pynput 编写了端到端 UI 自动化测试脚本 `1287.py`（同时置于 `TeXmacs/tests/python/1287.py`）：
1. 启动 Mogan STEM；
2. 按 `Ctrl+T` 从启动页新建标签页进入文档编辑状态（唤出完整文档菜单栏）；
3. 依次点击菜单：`文档` -> `颜色` -> `背景色` -> `渐变`；
4. 成功调出「Gradient selector」（渐变选择器）对话框；
5. 截取对话框截图（保存在 `devel/1287_gradient_dialog.png` 及裁剪图 `devel/1287_gradient_dialog_crop.png`）；
...
- **端到端 UI 自动化测试**：`1287.py`（及 `TeXmacs/tests/python/1287.py`）
  ...
  - 截取最终效果图保存在 `devel/1287_gradient_dialog.png` 与 `devel/1287_document_gradient.png`。
