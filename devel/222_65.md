...
### What
为 footer 的结构类快捷键反查补齐 command object 转换，并新增测试确认 `kbd-find-inv-binding` 对不同参数形式的行为差异。
...
测试 `TeXmacs/tests/222_65.scm` 证明：
- `"<alpha>"` 作为字符串可以命中
...
TeXmacs/tests/222_65.scm 新增参数形式测试，确认 object 形式能命中而字符串形式不能命中。
...
但进一步排查表明，即便复用菜单的显示链路，括号结构的核心问题仍然是“从结果树反推命令 source”不唯一。
因此当前结论是：
- `sqrt/frac/script/big operator/wide accent` 这类结构可继续依赖反查
- `around` / 定界符结构如果要得到与菜单完全一致的显示，不能只靠结果树逆向，需要额外的来源信息或单独的显示策略
