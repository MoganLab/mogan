...
3. 点击进度对话框的 **Cancel** 按钮 → 对话框应关闭，**不**弹出错误提示
4. **立即再次点击同一个模板** → 应重新弹出下载进度对话框
5. 等待下载完成 → 模板应正常打开，**不应出现多个下载框或多次打开文档**
...
在这段超时期间：
- 旧的 `QTMTemplateOpener` 对象仍然卡在 `loop.exec()` 中
- 旧对象仍然连接着 `TemplateManager` 的 `downloadCompleted` / `downloadFailed` 信号
- 用户点击"重新下载"后，新的下载完成时会 emit `downloadCompleted`
- **新旧所有卡住的 opener 都会收到该信号**，导致 `loadFromLocalPath_` 被多次执行

取消 N 次就会累积 N 个"幽灵 opener"，下次成功下载时同时触发 N+1 次完成处理。
...
QProgressDialog::canceled
  → QTMTemplateOpener::cancelDownload (templateId)
...
**重新下载（启动新请求前清理旧请求）：**
```
QTMTemplateOpener::openTemplate
...
- 用户点击取消 → `cancelDownload` emit `downloadFailed` → `QEventLoop` 立刻退出 → `QTMTemplateOpener` 立即析构，不残留
- 重新下载同一个模板时，旧的 opener 已彻底销毁，不会"幽灵"接收新下载的完成信号
