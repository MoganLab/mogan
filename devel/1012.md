...
本次改动分为 4 个 commit，每个 commit 只包含一类改动：

1. `[1012] 更新任务文档` — `devel/1012.md`
2. `[1012] TemplateCenter 核心代码改动` — `src/Mogan/TemplateCenter/*`、`TeXmacs/templates/categories.scm`
3. `[1012] Startup UI 相关改动` — `src/Plugins/Qt/QTMStartupTabWidget.*`、`QTMTemplateOpener.*`、`QTMTemplatePage.*`
4. `[1012] 测试相关改动` — `tests/Mogan/TemplateCenter/*`、`tests/Mogan/Startup/startup_tab_widget_test.cpp`
...
4. **MD5 文件校验**：下载后计算 MD5，本地可用性检查时验证文件完整性
5. **代码审查修复**：
   - 移除 `const_cast`，`isTemplateAvailableLocally` 改为非 `const`
...
4. **代码质量**：review 中发现 `const_cast`、脆弱的增量检测逻辑等问题需要修复
