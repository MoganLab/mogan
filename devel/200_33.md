...
## 2026/04/20 用户中心按钮优化
### 如何测试
1. **社区版测试**：设置 `is_community=true`，标题栏显示登录图标（无文字），点击直接跳转商业版官网
2. **商业版未登录状态**：设置 `is_community=false`，启动后应看到蓝色"未登录"文字，点击打开登录弹窗
3. **商业版已登录状态**：登录成功后，按钮应只显示图标，不显示用户名
4. **主题切换测试**：分别测试日间主题和夜间主题，检查未登录文字颜色是否正确（日间蓝色 `#0078d4`，夜间亮蓝 `#4fc3f7`）

### What
1. **社区版简化逻辑**：社区版显示登录图标但无状态变化，点击直接跳转官网
2. **商业版完整功能**：商业版显示"未登录"/图标状态，点击打开登录弹窗
2. **未登录状态文字样式优化**：未登录时文字显示蓝色（日间 `#0078d4`，夜间 `#4fc3f7`），已登录时使用常规颜色
3. **登录后不显示用户名**：已登录状态只显示图标，不显示用户名文字；未登录状态显示"未登录"
4. **按钮padding调整**：左padding从10px减到6px，使"未登录"文字不溢出
