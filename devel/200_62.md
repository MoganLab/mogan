...
## 2026/05/07 主动备份成功后通知云备份主动上传接口

### What

主动本地备份成功后通过 `auto-backup-upload-buffer` 通知云备份主动上传接口。

### Why

保存、另存为、PDF 导出这类用户主动操作完成后，本地备份已经写入 manifest。此时只需要通知云备份模块处理上传，上传实现仍由云备份模块负责。

### How

- 保存、另存为、PDF 导出路径在 `auto-backup-buffer` 返回后，统一把 `backup-result` 传给 `auto-backup-upload-buffer`。
- `auto-backup-upload-buffer` 从 `noop` 改为薄封装：仅当本地备份结果为 `'backup` 时，导入 `(session auto-cloud-backup)` 后零参数调用 `auto-cloud-backup-start-action-worker`。
- 不自行实现上传逻辑，也不调用通用 `auto-cloud-backup-start-worker`。
...
- 未新增 UI。`auto-backup-buffer` 后续仍走现有 manifest、md5 去重和 retention 逻辑。
