...
| 阶段 | 14:41（原始） | 14:58（删 Benchmark 后） | 15:02（再关闭 lazy-keyboard-force） |
|---|---|---|---|
| 内核引导 | ~520ms | ~517ms | ~473ms |
| Benchmark 1/2 | ~220ms | 已删 | 已删 |
| lazy-keyboard-force | ~1.1s | ~1.23s | ~1ms（已临时关闭） |
| resume | 173ms | 222ms | 1064ms |
| OPEN → STARTUP | ~1.6s | ~1.65s | ~1.29s |
...
- **启动页模式下推迟 `resume()` 的 chrome 重建**：首屏不依赖编辑器，
  menu_main/menu_icons ×5/notification_bar/side_tools 在 startupTabMode 下
  全部不可见，应跳过或推迟到首次打开文档时重建——这是当前 resume 1064ms
  的主要部分
