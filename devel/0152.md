...
3. 重置 `browseDragActive_ = false`
4. `browseDragging_ = true`
5. 消耗事件

**MouseMove（左键按住）**：
1. 计算鼠标全局位置与 `browseDragStartPos_` 的曼哈顿距离
2. 如果距离超过 `QApplication::startDragDistance()`：
   - `browseDragActive_ = true`
   - 设置光标为 `ClosedHandCursor`
3. 如果 `browseDragActive_` 为 true：
   - 计算垂直偏移 `deltaY = mouseEvent->globalPosition().toPoint().y() - browseDragStartPos_.y()`
   - 新滚动位置 = `browseDragStartY_ - deltaY`（grab-and-pull：鼠标向下拖动 → 抓住页面向下拉 → 滚动条值减小）
   - `verticalScrollBar()->setValue(newValue)`
4. 消耗事件

**MouseButtonRelease（左键）**：
1. `browseDragging_ = false`
2. 如果 `browseDragActive_` 为 false：
   - 视为单击（目前可留空，未来用于链接跳转）
3. 恢复光标为 `OpenHandCursor`
4. 消耗事件
...
2. 速度衰减：`velocity *= 0.85`（每帧衰减 15%）
3. 停止条件：`|velocity| < 0.3 px/ms`
