...
## 任务相关的代码文件
- `src/Edit/Modify/edit_table.cpp` - `table_default_hyphen_enabled` 及其调用点
...
### 非确定性测试（文档验证）
1. 启动 Mogan，新建一个空白文档
2. 插入一个行数较少（≤10 行）的表格，确认其默认不跨页（无 `table-hyphen`）
3. 插入一个行数超过 10 行的表格，输入内容直到跨页，确认表格在分页处自动断开

## What

将「插入表格时默认开启表格分页」改为按行数判定：仅当插入表格的行数超过
10 行时才在 `TFORMAT` 中添加 `table-hyphen = "y"`，否则默认不分页。

## Why

此前默认对所有新插入表格开启 `table-hyphen`。但大多数表格行数很少，本
不会跨页，无条件开启分页改变了小表格的行为预期；只有行数较多、可能跨
页的表格才真正需要默认分页。

## How

`table_default_hyphen_enabled` 增加行数参数：
...
调用点：
- `make_table`：以请求行数判定；若因 format limits 扩行重建
  （`max (nr_rows, i1)`），按最终行数重新判定
- `make_subtable`：以请求行数判定

单元测试更新 `table_default_hyphen_enabled` 新签名，并补充边界用例：
10 行不开、11 行开、math 模式无论行数均不开。
