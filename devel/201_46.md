...
6. 自动备份勾选框测试：
   - 勾选“开启自动备份”，点击跳过登录，会有提示弹窗，点击提示弹窗中的取消会返回引导窗口，点击关闭自动备份会直接进入软件。进入软件后，运行(get-preference "autobackup")，查看输出是否为off。
   - 勾选“开启自动备份”，点击登录并完成登录流程。进入软件后，运行(get-preference "autobackup")，查看输出是否为on。
   - 取消勾选“开启自动备份”，任意点击按钮进入软件，运行(get-preference "autobackup")，查看输出是否为off。
