...
- `moebius/xmake.lua`(原 1129 遗漏:`add_requires("s7")` 与
  `libmoebius` 的 `add_packages("s7")` 由 s7 改为 goldfish)
...
## 7 补遗:迁移 `moebius/xmake.lua`

### 7.1 背景

原 1129 删除本地 `xmake/packages/s/s7/` 后,只迁移了 `xmake.lua` 与
`xmake/tests.lua` 两处引用,**漏掉了 `moebius/xmake.lua`**:

- `add_requires("s7", {system=false})`
- `libmoebius` 的 `add_packages("s7")`

本地 `liii-repo` 不再定义 `s7` 包,xmake 便回退到全局 `xmake-repo`
的上游 `s7` 包。上游包相对本地删除的版本多出一个 `WITH_MAIN` 的
`s7` 二进制 target,并在 Linux 上强制
`add_ldflags("-static", "-static-libgcc", {force = true})`,
导致无静态库的发行版(如 Fedora)在链接该二进制时报
`找不到 -lstdc++/-lm/-lc`。

`moebius/src/` 本身不引用任何 s7 头文件/符号(`libmoebius` 的
`add_packages("s7")` 实为历史遗留的死依赖),因此此前一直潜伏。

### 7.2 修复

将 `moebius/xmake.lua` 的上述两处 `s7` 同步改为 `goldfish`,与
`xmake.lua` / `xmake/tests.lua` 的迁移保持一致。并清理
`~/.xmake/cache/packages/.../s/s7/` 残留缓存,避免回退到上游包。

### 7.3 验证

- `grep -rn 'add_requires.*"s7"\|add_packages.*"s7"' xmake.lua xmake/*.lua moebius/xmake.lua`
  无匹配。
- `xmake f -c --yes -m releasedbg` 配置通过,安装列表不再出现 `s7`。
- `xmake b libmoebius` 构建通过(`libmoebius.a` 归档成功)。
