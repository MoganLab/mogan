...
### 关联变更（[1260.md](1260.md)）

[1260] 修复画布上下空隙时改动了同一处 `pager_rep::make_pages()` 页面网格循环：
`PAGE_BORDER != "none"` 分支内，文档首行页面不加上衬边（`t=0`）、末行页面不加下衬边
（`b=0`），消除 paper 文档首尾露出的背景色空隙。Book 模式为 `page-border="none"`，
走 else 分支，不受影响；`vgap` 垂直间隙逻辑未动，页面间等距间隙保持不变。
