...
- **Linux (X11 + NVIDIA) 点击 Pick screen color 段错误（人工验证反馈，栈顶 `QRhi::endOffscreenFrame`）**：根因是 `pickScreenColor()` 在 QML 事件处理器内对取色 overlay 跑嵌套 `exec()`，嵌套事件循环期间宿主弹窗的 QQuickWidget 被迫重绘，QRhi 离屏帧在 nvidia GL 驱动下崩溃。修复：取色改为异步——`pickScreenColor()` 只发起取色（overlay 用 `open()` + ApplicationModal，不再嵌套 exec），结果经新信号 `screenColorPicked(QString)` 回流，QML 侧用 `Connections` 接收；取色期间隐藏宿主弹窗（QQuickWidget 不再渲染），结束后恢复。bridge 析构负责销毁未关闭的 overlay，避免遗留全屏无父浮层。新增 `tests/Plugins/Qt/color_picker_bridge_test.cpp` 覆盖异步流程（点击回传合法 hex、Esc 取消回空串、宿主隐藏/恢复）。
- **Wayland 点击 Pick screen color 全屏黑屏（人工验证反馈）**：Wayland 不允许应用任意抓屏，`grabWindow(0)` 返回空图或全黑快照，全屏 overlay 遂显示全黑。修复：新增 `canPickScreen` 只读属性（platformName 以 wayland 开头即 false），QML 据此隐藏「Pick screen color」按钮；`pickScreenColor()` 在不支持平台立即回空串兜底。
